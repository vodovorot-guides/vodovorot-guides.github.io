...
Неприятный нерф длительности, который в Shadowlands частично перекрывается {{ site.data.spells.fe }} за счет {{ site.data.spells.fe2 }}. Но не в препатче, так как мы будем ограничены 50 уровнем.
...
Все три изменения выше, по сути, это внутренний баланс урона специализации. Как это повлияет на общий урон Элема нужно будет смотреть уже на релизе Shadowlands.

• {{ site.data.spells.lava_surge }} – в [официальных патчноутах](https://worldofwarcraft.com/en-us/news/23529209/shadowlands-pre-expansion-patch-notes) можно увидеть, что шанс срабатывания понижен с **15%** до **10%**. Мы не знаем, ошибка это (потому что в тех патчноутах много ошибок), так задумано для Shadowlands (на Бете этого нет) или это только для препатча, как компенсация Азеритового трейта {{ site.data.spells.potential }}, который помимо увеличения урона от {{ site.data.spells.lvb }} также увеличивает шанс прока {{ site.data.spells.lava_surge }} на 3%. Ждем дальнейшей информации.
...
• {{ site.data.spells.ch }} – стоит чуть дороже {{ site.data.spells.hs }}, а также дольше кастуется. Будет полезно в те моменты боя, когда невозможно атаковать, но необходимо переживать урон – в рейде есть несколько боев с подобными фазами. Но в бою мы вряд-ли будем часто использовать это заклинание.
...
Самый заметный нерф это ограничение {{ site.data.spells.eye_storm }}  на 6 целях. Его можно будет заметить в Мифик+, а также в Ни’алоте на Вексионе, Коллективном разуме и Панцире Н’Зота. Подробнее про ограничение количества целей на АОЕ [в этой статье](https://stormkeeper.ru/common/target_cap.html).
