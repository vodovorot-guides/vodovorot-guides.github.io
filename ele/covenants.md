...
**Помните, что правки способностей ковенантов идут практически каждую неделю и сейчас нет смысла ничего выбирать, для этого следует дождаться релиза.** Тоже самое касается медиумов – сейчас мало смысла о них говорить, так как
...
* Отхил срабатывает от {{ site.data.spells.hst }}.  
...
* Флакон имеет три заряда и НЕ делит время восстановления с зельем лечения. 
...
{{ site.data.covenants.pw }} — поражает одну цель, нанося небольшой урон противнику и применяя к нему {{ site.data.spells.fs }}. Применяется мгновенно. Время восстановления 45 сек. Ваш следующий {{ site.data.spells.lvb }}. также применится ко всем целям под {{ site.data.spells.fs }}..
...
* Лучше всего себя показывает во время боя против разрозненных целей, где необходимо поддерживать сразу несколько {{ site.data.spells.fs }}.  
* {{ site.data.covenants.pw }} может давать достаточно сильный бурст на Арене, к тому же чаще чем другие способности ковенантов.  
* {{ site.data.conduits.wave }} – проводник, усиливающий эту способность. Дает 20-40% шанс сбросить кулдаун {{ site.data.covenants.pw }} при её использовании. Рандомный шанс срабатывания понравится не всем, но если напрокает – урон вырастает достаточно ощутимо, особенно на АоЕ.
...
* На самом деле наносит урон 7 раз, а не 6. Подсказка на Вовхеде не учитывает «initial tick», то есть урон на 0 секунде использования.
