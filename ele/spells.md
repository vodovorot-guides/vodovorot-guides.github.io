---
title: Способности Шамана Стихии
layout: page
last_update: 2022-08-11
wow: 10.0
toc: true
author: Amani
description: В этой статье вы найдете подробное описание всех способностей и многих основных талантов Шамана Стихии. Прикладная энциклопедия по всем аспектам механики и нюансам заклинаний!
image: /assets/img/logos/spells.png
---

# Вступление

* Шаман Стихии, он же Элем Шаман – боец дальнего боя, полагающийся на заклинания и помощь стихий. Основная характеристика – Интеллект.
* Элем использует кольчужные доспехи, а из оружия предпочитает щит с одноручным молотом, кинжалом или кистевым оружием, а также посохи.
* Шаман Стихии использует школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span>, <span style="color:#f16f25;font-size:1em;">**Огня**</span> и <span style="color:#8fd5fe;font-size:1em;">**Льда**</span>, а также наносит <span style="color:#FFFFFF;font-size:1em;">**Физический**</span> урон (только [Землетрясение](https://ru.wowhead.com/spell=61882)).
* Дальность всех атакующих заклинаний – **40** метров. Исключениями являются некоторые вспомогательные способности, которые имеют дальность **30** метров.
* Основной ресурс Шамана Стихии – [Водоворот](https://ru.wowhead.com/spell=343725), а дополнительный – **Мана** (используется для лечения и вспомогательных заклинаний).

# Базовые способности

Способности в этом разделе доступны без взятия каких-либо талантов.

## Ресурсы


<a href="https://ru.wowhead.com/spell=343725" target="blank" data-wh-icon-size="small" > **Водоворот**</a> – основной ресурс, генерируется одними заклинаниями и тратится на другие.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>

> Maelstrom, MS, МС, Рага, Энергия.

* Энергия Водоворота генерируется следующими заклинаниями:
1. {{ site.data.spells.lb }} – **8** ед. энергии Водоворота базово и **3** ед. при проке Искусности.
2. {{ site.data.spells.lvb }} – **10** ед. энергии Водоворота базово и **4** ед. при проке Искусности.
3. {{ site.data.spells.cl }} – **4** ед. энергии Водоворота базово и **3** ед. при проке Искусности (за каждую цель).
4. {{ site.data.spells.eb }} – **30** ед. энергии Водоворота базово и **15** ед. при проке Искусности.
5. {{ site.data.spells.if }} – **25** ед. энергии Водоворота базово и **12** ед. при проке Искусности.
6. {{ site.data.spells.frs }} под баффом от [Ледяной ярости](https://ru.wowhead.com/spell=210714) – **8** ед. энергии Водоворота.
7. {{ site.data.spells.fs }} с талантом [Жгучее пламя](https://ru.wowhead.com/spell=381782) генерирует **1/2** ед. энергии Водоворота за тик.
* Со взятым талантом [Поток силы](https://www.wowhead.com/ru/spell=385923/) генерация энергии Водоворота [Молнии](https://ru.wowhead.com/spell=188196) составляет 
**10** единиц при касте и **4** единицы при проке Искусности, а [Выброса лавы](https://ru.wowhead.com/spell=51505) – **12** и **5** единиц соответственно.
* Энергия Водоворота тратится на следующие заклинания:
  * {{ site.data.spells.es }} – наносит средний урон одной цели, применяется мгновенно, стоит **60** ед. энергии Водоворота.
  * [Удар духов стихий](https://www.wowhead.com/ru/spell=117014) – наносит большой урон одной цели, кастуется **2** секунды, дает баффы на характеристики и стоит **90** ед. энергии Водоворота. При взятии этого таланта заменяет на панели {{ site.data.spells.es }}.
  * {{ site.data.spells.quake }} – наносит средний урон на АоЕ в течение **6** секунд, применяется мгновенно и стоит **60** ед. энергии Водоворота. Несколько [Землетрясений](https://ru.wowhead.com/spell=61882) стакаются.
* Стоимость спендеров снижается с помощью таланта [Око бури](https://www.wowhead.com/ru/spell=381708) – на **5/10** для [Земного шока](https://ru.wowhead.com/spell=8042) и [Землетрясения](https://ru.wowhead.com/spell=61882) и на **7/15** для [Удара духов стихий](https://www.wowhead.com/ru/spell=117014).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=168534" target="blank" data-wh-icon-size="small" > **Искусность: волнение стихий**</a> – дает шанс некоторым заклинаниям поразить цель повторно, нанеся меньше урона и давая меньше энергии Водоворота.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Мастери, Иск, Перегрузка, Дублирование, Волнение стихий.

* Базовый шанс на перегрузку, без учета характеристик, составляет **15%**.
* Чтобы поднять шанс прока перегрузки на **1%**, требуется **18.6** ед. Искусности.
* Урон дублированного заклинания при проке перегрузки снижен до **85%** от базового. При взятии таланта [Камера эхо](https://www.wowhead.com/ru/spell=382032) урон повышается до **93%/100%**.
* Критический удар рассчитывается для прока Искусности отдельно.
* Эффективность [Искусности](https://ru.wowhead.com/spell=168534) для [Цепной молнии](https://ru.wowhead.com/spell=188443) **снижена до 33%**. То есть, если у вас **60%** Искусности, то [Цепная молния](https://ru.wowhead.com/spell=188443) имеет только **20%** на прок перегрузки.
* Перегрузка вылетает не сразу после завершения каста заклинания, а примерно через **1** секунду.
* Если сразу же после каста заклинания скастовать, например, {{ site.data.spells.es }}, то перегрузка вылетит уже после траты энергии Водоворота и энергия не уйдет в оверкап.
* Талант [Горы не вечны](https://www.wowhead.com/ru/spell=381726) позволяет вызывать срабатывание искусности [Земному шоку](https://ru.wowhead.com/spell=8042), [Землетрясению](https://ru.wowhead.com/spell=61882) и [Удару духов стихий](https://www.wowhead.com/ru/spell=117014), нанося при этом в два раза меньше урона.
* Перегрузка [Выброса лавы](https://ru.wowhead.com/spell=51505) не дает бафф от таланта {{ site.data.spells.mote }}, но перегрузка [Удара духов стихий](https://ru.wowhead.com/spell=117014) гарантированно дает второй бафф на статы, отличный от первого.
* Все таланты, увеличивающие урон основного заклинания, влияют также и на его прок искусности.
* Искусность не влияет на тотемы и лечащие заклинания.
* При использовании [Первозданной волны](https://ru.wowhead.com/spell=375982) шанс на прок Искусности расчитывается отдельно на каждую цель. 
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

## Атакующие способности


<a href="https://ru.wowhead.com/spell=188196" target="blank" data-wh-icon-size="small" > **Молния**</a> – наносит средний урон магии природы одной цели и генерирует энергию Водоворота.
<p></p>
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>

> (Lightning Bolt, Bolt, LB, Болт, ЛБ)

* Время произнесения – **2** секунды (только после **8** уровня и открытия пассивной способности [Молния: Ранг 2](https://ru.wowhead.com/spell=318044), до этого **2.5** сек). Талант [Безжалостное бедствие](https://www.wowhead.com/ru/spell=382685) дополнительно сокращает время произнесения до **1.75** секунд.
* Наносит урон от сил <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1%** маны.
* Генерирует **8** ед. энергии Водоворота и дополнительно **3** ед. при проке [Искусности: волнение стихий](https://ru.wowhead.com/spell=168534). С талантом [Поток силы](https://ru.wowhead.com/spell=385923) – **10** и **4** ед. энергии Водоворота соответственно.
* [Молния](https://ru.wowhead.com/spell=188196) не имеет времени полета снаряда (потому что [очень быстро](https://ru.wikipedia.org/wiki/%D0%9C%D0%BE%D0%BB%D0%BD%D0%B8%D1%8F) двигается).
* С талантом [Хранитель бурь](https://ru.wowhead.com/spell=191634) превращается в бурст способность – кастуется мгновенно, урон увеличивается на **150%** и вылетает гарантированный прок [Перегрузки](https://ru.wowhead.com/spell=168534). С этим баффом {{ site.data.spells.lb }} применяется в том числе в бою против **2** целей.
* Усиливается следующими талантами:
  * [Улучшенная молния](https://www.wowhead.com/ru/spell=381674/) – добавляет **20%** урона.
  * [Гнев природы](https://www.wowhead.com/ru/spell=381655) – повышает на **2/4%** шанс Критического удара.
  * [Хранитель бурь](https://www.wowhead.com/ru/spell=191634) – добавляет **150%** урона, позволяет применять мгновенно и гарантирует срабатывания [Перегрузки](https://ru.wowhead.com/spell=168534).
  * [Призыв грома](https://www.wowhead.com/ru/spell=378241/) – добавляет **15%** урона.
  * [Безжалостное бедствие](https://www.wowhead.com/ru/spell=382685) – сокращает время применения на **0.25** секунд.
  * [Вспышка молнии](https://www.wowhead.com/ru/spell=381936) – дает снижение кулдауна природных заклинаний за каждый каст.
  * [Электрический шок](https://www.wowhead.com/ru/spell=382086) – добавляет **15%** урона.
  * [Грозовой разрядник](https://www.wowhead.com/ru/spell=210689/) – добавляет **20%** урона за каждый наложенный дебафф.
  * [Властелин стихий](https://www.wowhead.com/ru/spell=16166/) – добавляет **20%** урона.
  * [Прилив мощи](https://www.wowhead.com/ru/spell=262303/) – добавляет **2** гарантированных срабатывания [Перегрузки](https://ru.wowhead.com/spell=168534).
  * [Мощь Водоворота](https://www.wowhead.com/ru/spell=191861) – добавляет **1** гарантированное срабатывание [Перегрузки](https://ru.wowhead.com/spell=168534).
  * [Камера эхо](https://www.wowhead.com/ru/spell=382032) – увеличивает урон от [Перегрузки](https://ru.wowhead.com/spell=168534) на **15%**.
  * [Поток силы](https://www.wowhead.com/ru/spell=385923/) – увеличивает генерацию энергии Водоворота.

</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=188389" target="blank" data-wh-icon-size="small" > **Огненный шок**</a> – наносит небольшой мгновенный урон одной цели и оставляет на цели дебафф, наносящий средний урон в течение **18** секунд.

<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>

> Flame Shock, FS, Флейм шок, ФШ, Фаер шок (не стоит), Шок (одно из значений), Дотка.

* Наносит небольшой урон при наложении, а также средний урон за **18** секунд.
* Максимум [Огненный шок](https://ru.wowhead.com/spell=188389) может быть наложен на **6** целей.
* Наносит урон от магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и стоит **1.5%** маны.
* Периодический урон наносится раз в **2** секунды, это время сокращается за счет Скорости. С талантом [Пламя Котла](https://ru.wowhead.com/spell=378266) урон наносится на **15%** быстрее.
* Имеет время восстановления **6** секунд, которое **НЕ** снижается Скоростью, в отличие от Шамана Совершенствования. С талантом [Пламя Котла](https://ru.wowhead.com/spell=378266) кулдаун составляет **4.5** секунд.
* Скорость добавляет дополнительный тик в конце действия эффекта, ослабленный в зависимости от недостающего времени до «полного» тика.
* Является **магическим** эффектом, то есть может быть развеяно противником с помощью способностей [Возрождение духа](https://ru.wowhead.com/spell=77130), [Очищение](https://ru.wowhead.com/spell=527), [Детоксикация](https://ru.wowhead.com/spell=115450) и другими.
* Не имеет времени полета снаряда, то есть накладывается мгновенно.
* При активном [Элементале огня](https://ru.wowhead.com/spell=198067) длительность [Огненного шока](https://ru.wowhead.com/spell=188389) увеличена **вдвое** и составляет **36** секунд. Данный эффект **не работает** с [Элементалем бури](https://ru.wowhead.com/spell=192249).
* Если обновить {{ site.data.spells.fs }}, когда до спадения остается менее **5.4** секунд, то это время действия добавится к новому наложению, продлевая шок вплоть до **23.4** секунд (так называемое [«окно пандемии»](https://wowwiki-archive.fandom.com/wiki/Pandemic)).
* Все время свыше **5.4** секунд на момент обновления [Огненного шока](https://ru.wowhead.com/spell=188389) **не добавится** к новому эффекту.
* При активном [Элементале огня](https://ru.wowhead.com/spell=198067) {{ site.data.spells.fs }} можно обновлять на **10.8** секунде, что даст [Огненный шок](https://ru.wowhead.com/spell=188389) длительностью **46.8** секунд.
* Также при активном [Элементале огня](https://ru.wowhead.com/spell=198067) {{ site.data.spells.fs }} тикает на **25%** чаще, то есть раз в **1.5** секунды (без учета Скорости). Данный эффект **не работает** с [Элементалем бури](https://ru.wowhead.com/spell=192249).
* Усиливается следующими талантами:
  * [Лед и пламя](https://ru.wowhead.com/spell=382886) –  **3%** урона.
  * [Волна лавы](https://ru.wowhead.com/spell=77756) – каждый тик имеет **10%** шанс восстановить [Выброс лавы](https://ru.wowhead.com/spell=51505) и сделать его мгновенным.
  * [Зов огня](https://ru.wowhead.com/spell=378255) – добавляет **10%** урона.
  * [Пламя Котла](https://ru.wowhead.com/spell=378266) – снижает КД на **1.5** секунд и увеличивает скорость тиков на **15%**.
  * [Улучшенное оружие языка пламени](https://ru.wowhead.com/spell=382027) – добавляет **5%** урона. 
  * [Жгучее пламя](https://ru.wowhead.com/spell=381782) – каждый тик дает **1/2** ед. энергии Водоворота.
  * [Огненная гибель "Усмирителя небес"](https://ru.wowhead.com/spell=378310) – увеличивает шанс Критического удара на **50%** и каждый крит снижает КД  [Элементале огня](https://ru.wowhead.com/spell=198067) или [Элементаля бури](https://ru.wowhead.com/spell=192249) на **1** секунду.
  * [Камера магмы](https://ru.wowhead.com/spell=381932) – каждый тик увеличивает урон следующего [Земного шока](https://ru.wowhead.com/spell=8042), [Землетрясения](https://ru.wowhead.com/spell=61882) или [Удара духов стихий](https://ru.wowhead.com/spell=117014) на **0.8%/1.5%**, суммируется до **20** раз.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=318038" target="blank" data-wh-icon-size="small" > **Оружие языка пламени**</a> – усиливает оружие ближнего боя, нанося дополнительный урон от огня.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Flametongue Weapon, Флеймтонг, огненная чарка.

* Усиливается талантом [Улучшенное оружие языка пламени](https://ru.wowhead.com/spell=382027), позволяя огненным заклинаниям наносить дополнительно **5%** урона. Без этого таланта [Оружие языка пламени](https://ru.wowhead.com/spell=318038) бесполезно для Элема.
* Не дает использовать руны на оружие, например [Жужжащую руну](https://ru.wowhead.com/item=194821) или [Воющую руну](https://ru.wowhead.com/item=194817).
* Добавляет огненную анимацию вокруг оружия персонажа.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=73899" target="blank" data-wh-icon-size="small" > **Стихийный удар**</a> – наносит небольшой урон в ближнем бою. Бесполезная способность для шамана Стихии. 

<hr style="height:1px;background-color:#bbb">
<p></p>


## Вспомогательные способности


<a href="https://ru.wowhead.com/spell=8004" target="blank" data-wh-icon-size="small" > **Исцеляющий всплеск**</a> – быстрое лечение одной цели.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Healing Surge, HS, ХС, Всплеск, Флешка, Сурдж, Хилка.

* Исцеляет одну цель на **280%** от силы заклинаний
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **25%** маны.
* В подсказке на Wowhead указано **248%** от силы заклинаний, но у Элема отхил этой способности увеличен на **13%** за счет пассивной ауры – [Шаман - Стихии](https://ru.wowhead.com/spell=137040).
* Кастуется **1.5** секунды.
* В отличие от атакующих заклинаний, [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004) можно применять к союзнику, который стоит за вашей спиной.
* На [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004) оказывают влияние все характеристики, кроме Искусности.
* Крайне полезное заклинание для экстренного отхила как себя, так и союзника. Снимает один стак [Мучительного](https://ru.wowhead.com/affix=12) аффикса, а также полезно на [Взрывном](https://ru.wowhead.com/affix=11) аффиксе для отхила после смерти пака.
* Усиливается следующими талантами:
  * [Гнев природы](https://www.wowhead.com/ru/spell=381655) – повышает на **2/4%** шанс Критического удара.
  * [Точное озарение](https://www.wowhead.com/ru/spell=381666) – увеличивает отхил на **15%/30%** и снижает стоимость после применения [Огненного шока](https://ru.wowhead.com/spell=188389).
  * [Кружащие потоки](https://www.wowhead.com/ru/spell=378094) – увеличивает отхил на **13%/25%** после применения [Тотема исцеляющего потока](https://www.wowhead.com/ru/spell=5394).
  * [Освежающие воды](https://www.wowhead.com/ru/spell=378211) – увеличивает отхил по вам на **25%**.
  * [Первозданная ярость](https://www.wowhead.com/ru/spell=378193/) – критический эффект увеличивается до **250%** вместо стандартных **200%**.
  * [Кружашие потоки](https://www.wowhead.com/ru/spell=378094) – установка [Тотема исцеляющего потока](https://www.wowhead.com/ru/spell=5394) усиливает **3** следующие заклинания исцеления на **13%/25%**.


</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=2484" target="blank" data-wh-icon-size="small" > **Тотем оков земли**</a> – замедление врагов в указанной области на **50%**.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Earthbind Totem, Тотем замедления.

* При использовании устанавливается тотем в выбранном месте, замедляющий окружающих врагов.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **2.5%** маны.
* Замедляет все цели вокруг тотема на **50%**.
* Радиус действия – **10** метров вокруг тотема.
* Радиус установки тотема – **40** метров вокруг персонажа.
* Эффект замедления от тотема накладывается раз в **1** секунду.
* Эффект пропадает при покидании целью радиуса действия.
* Длительность составляет **20** секунд
* Время восстановления – **30** секунд.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Усиливается следующими талантами:
  * [Сила тотемов](https://www.wowhead.com/ru/spell=381867/) – сокращает кулдаун на **2/4** секунды.
  * [Проецирование тотемов](https://www.wowhead.com/ru/spell=108287) – переносит установленные тотемы в указанное место.
  * [Тотемное средоточие](https://www.wowhead.com/ru/spell=382201) – увеличивает радиус и время действия тотема.
  * [Возвращение тотемов](https://www.wowhead.com/ru/spell=108285) – сбрасывает кулдаун последнего примененного тотема.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=2825" target="blank" data-wh-icon-size="small" > **Жажда крови**</a> / <a href="https://ru.wowhead.com/spell=32182" target="blank" data-wh-icon-size="small" > **Героизм**</a> – бафф, повышающий Скорость союзников на **30%** на **40** секунд. Не сочетаается с другими подобными заклинаниями.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Bloodlust, Heroism, BL, Hero, БЛ, Гера, Рост.

* Используйте это заклинание с умом, предварительно скоординировав его активацию с Рейд Лидером или вашей группой.
* Не имеет ГКД.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **21.5%** маны.
* **Дает больше Скорости, чем написано в описании способности** – помимо **30%**, добавляет также **30%** от вашего значения Скорости. 
* Если у вас без баффов **20%** Скорости, то под [Жаждой крови](https://ru.wowhead.com/spell=2825) она станет равна не **50%**, а **56%** (**20%** + **30%** + (**20%** * **0.3**)).
* Союзники, получившие этот эффект, не могут повторно получить подобный эффект в течение **10** минут за счет дебаффа [Пресыщение](https://ru.wowhead.com/spell=57724).
* Время восстановления способности меньше, чем длительность дебаффа [Пресыщение](https://ru.wowhead.com/spell=57724), **5** минут против **10** минут.
* Увеличивает размер модели персонажа на **30%**.
* Имеет практически неограниченный радиус применения (**50000** метров), позволяя в открытом мире баффнуть союзника практически на другой стороне континента.
* [Жажда крови](https://ru.wowhead.com/spell=2825) и [Героизм](https://ru.wowhead.com/spell=32182) – это одна и та же способность, которая отличается только звуком применения. [Жажда крови](https://ru.wowhead.com/spell=2825) есть только у Орды, а [Героизм](https://ru.wowhead.com/spell=32182) – у Альянса.
* Бафф является магическим эффектом и его можно рассеять.
* Аналоги других классов – [Искажение времени](https://ru.wowhead.com/spell=80353) Мага, [Древняя истерия](https://ru.wowhead.com/spell=90355) Охотника и [Ярость Аспектов](https://www.wowhead.com/ru/spell=390386) Пробудителя.
* Другие классы могут использовать [Барабаны смертельной свирепости](https://ru.wowhead.com/item=172233), однако их эффект **в два раза слабее**.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=2645" target="blank" data-wh-icon-size="small" > **Призрачный волк**</a> – превращение в призрачного волка, который увеличивает скорость передвижения, но применение заклинаний отменяет эффект.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Ghost Wolf, Волк, Форма волка, Пес, Раптор, Динозавр.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Увеличивает скорость передвижения на **30%**.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) скорость передвижения не может быть снижена менее чем до **100%**.
* Применение любого заклинания (кроме [Хождения по воде](https://ru.wowhead.com/spell=546) и [Астрального сдвига](https://ru.wowhead.com/spell=108271)) отменяет форму [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Не имеет времени восстановления.
* При входе в форму тратится ГКД, а при выходе из формы – нет.
* Форма [Призрачного волка](https://ru.wowhead.com/spell=2645) считается магическим эффектом и может быть рассеяна соответствующими заклинаниеми, типа [Развеивания магии](https://ru.wowhead.com/spell=370), [Рассеивания заклинаний](https://ru.wowhead.com/spell=528/) и других. [Чарокрад](https://ru.wowhead.com/spell=30449) Мага при этом снимет эффект формы, но не получит этот эффект для себя.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) вы считаетесь **Животным**, а не **Гуманоидом**:
  * Охотники видят вас на миникарте с помощью [Выслеживания животных](https://ru.wowhead.com/spell=210065) и могут законтролить с помощью [Отпугивания](https://ru.wowhead.com/spell=1513), а Друид – с помощью [Спячки](https://ru.wowhead.com/spell=2637).
  * Вас нельзя отследить с помощью [Выслеживания гуманоидов](https://ru.wowhead.com/spell=19883), а Паладин не сможет законтролить с помощью [Покаяния](https://ru.wowhead.com/spell=20066).
* Форму [Призрачного волка](https://ru.wowhead.com/spell=2645) можно принимать в облике духа после смерти, что позволит быстрее добраться до своего тела.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно собирать траву.
* Талант [Дух волка](https://ru.wowhead.com/spell=260878) дополнительно повышает скорость передвижения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645) на **20%**, а также дает снижение получаемого урона на **20%**. Эффект накапливается по **5%** за одну секунду нахождения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Талант [Громоподобные лапы](https://www.wowhead.com/ru/spell=378075) позволяет раз в **60** секунд при входе в форму [Призрачного волка](https://ru.wowhead.com/spell=2645) снять с себя все замедления и увеличить скорость передвижения на **25%** на **3** секунды.
* Внешний вид формы [Призрачного волка](https://ru.wowhead.com/spell=2645) можно изменить с помощью символов, которые делаются начертателями и покупаются на аукционе:
  * [Символ призрачного ящера](https://ru.wowhead.com/item=137287)
  * [Символ волка-призрака](https://ru.wowhead.com/item=43386/)
  * [Символ призрачного люпина](https://www.wowhead.com/ru/item=190378)
  * [Символ призрачного вульпина](https://www.wowhead.com/ru/item=190380)
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно танцевать, с помощью команды **/dance**.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно атаковать, нанося при этом базовый урон от оружия.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=20608" target="blank" data-wh-icon-size="small" > **Реинкарнация**</a> – воскрешение в случае смерти.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
* **Не является «боевым воскрешением»**, то есть не идет в счетчик БР в Рейде и в Мифик+.
* Уникальная способность шамана, не имеет аналогов у других классов.
* Время восстановления не сбрасывается при завершении рейдового боя и при начале Мифик+.
* При воскрешении у вас будет **20%** здоровья и маны, но на **46** уровне открывается пассивка [Улучшенная реинкарнация](https://ru.wowhead.com/spell=319352), которая увеличивает значение здоровья и маны после реса до **40%**.
* Талант [Переполняющая сила жизни](https://www.wowhead.com/ru/spell=381689) увеличивает максимальный запас здоровья на **8%** если [Реинкарнация](https://ru.wowhead.com/spell=20608) не на кулдауне, а при полном здоровье кулдаун [Реинкарнации](https://ru.wowhead.com/spell=20608) сокращается до **~17** минут.
* [Реинкарнация](https://ru.wowhead.com/spell=20608) не работает на Арене, но работает на РБГ.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=192106" target="blank" data-wh-icon-size="small" > **Щит молний**</a> – наносит урон атакующим в ближнем бою.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
>  Lightning Shield, LS, ЛС, Щит.  

* При использовании вы получаете бафф на **30** минут, каждый удар по вам в ближнем бою наносит атакующему **15%** урона от силы заклинаний и дает вам **5** ед. энергии Водоворота.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1.5%** маны.
* Возможно поддерживать только один щит стихий одновременно – либо [Щит молний](https://ru.wowhead.com/spell=192106), либо [Щит земли](https://ru.wowhead.com/spell=974). Талант [Кружащие стихии](https://www.wowhead.com/ru/spell=383010) снимает это ограничение.
* Талант [Ободряющие щиты](https://www.wowhead.com/ru/spell=382033/) увеличивает наносимый урон и генерацию энергии Водоворота [Щита молний](https://ru.wowhead.com/spell=192106) в два раза.
* Дает анимацию электрического шара, кружащего вокруг персонажа.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=2008" target="blank" data-wh-icon-size="small" > **Дух предков**</a> – воскрешение союзника вне боя.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Ancestral Spirit, Рес.

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **4%** маны.
* Время произнесения – **10** секунд.
* Воскрешается **1** союзник с **35%** здоровья и маны.
* Не имеет времени восстановления.
* Радиус воскрешения **40** метров, тогда как у массового воскрешения Рестора, [Древнее видение](https://ru.wowhead.com/spell=212048), **100** метров.
* Символ, который добавляет анимацию духа предков после воскрешения, – [Символ далеких предков](https://ru.wowhead.com/item=104127).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=6196" target="blank" data-wh-icon-size="small" > **Дальнее зрение**</a> – переносит точку обзора в указанное место.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Кастуется **2** секунды и длится **1** минуту.
* После каста можно применить это заклинание еще раз, переместив точку обзора в следующее место.
* Практически не имеет ограничения на дальность применения.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=556" target="blank" data-wh-icon-size="small" > **Астральное возвращение**</a> – дополнительный [Камень возвращения](https://ru.wowhead.com/item=6948), переносит вас в привязанную локацию.   
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Возвращает в ту же точку, куда назначен [Камень возвращения](https://ru.wowhead.com/item=6948), но позволяет делать это чаще.
* Имеет раздельное время восстановления с [Камнем возвращения](https://ru.wowhead.com/item=6948).
* Кастуется **10** секунд, но, в отличие от [Камня возвращения](https://ru.wowhead.com/item=6948), это время сокращается за счет Скорости.
* Можно использовать на ходу во время действия [Благосклонности предков](https://ru.wowhead.com/spell=79206).
* Время восстановления – **10** минут.
* Нельзя использовать в бою и на арене.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=546" target="blank" data-wh-icon-size="small" > **Хождение по воде**</a>  – позволяет союзнику ходить по воде.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
* При использовании накладывает бафф на **10** минут, который позволяет ходить по воде.
* Любой урон отменяет этот эффект.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Можно использовать не выходя из формы [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Кастуется мгновенно и имеет ГКД.
* Радиус действия [Хождения по воде](https://ru.wowhead.com/spell=546) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* ГКД от этой способности не распространяется на призыв маунтов. Поэтому можно использовать одновременно {{ site.data.spells.waterwalking }} и маунта: 
~~~
#showtooltip 
/cast [nomounted,nocombat] Хождение по воде
/run C_MountJournal.SummonByID(0)
~~~
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

# Общие таланты шамана

## Первый ряд


<a href="https://ru.wowhead.com/spell=1064" target="blank" data-wh-icon-size="small" > **Цепное исцеление**</a> – АоЕ отхил четырех союзников с долгим кастом. Практически бесполезный талант для Элема.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>

> Chain Heal, CH, Чайник (одно из значений), Цепнуха (одно из значений).

* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **30%** маны.
* Исцеляет три цели, первую на **210%** от силы заклинаний, вторую на **147%**, третью на **103%**.
* Суммарное количество отхила – **460%** от силы заклинаний.
* Кастуется **2.5** секунды.
* Является «улучшенным смарт-хилом», то есть хилит в первую очередь цели с наименьшим количеством здоровья.
* Радиус перескока отхила составлявет **15** метров.
* В отличие от атакующих заклинаний, [Цепное исцеление](https://ru.wowhead.com/spell=1064) можно применять к союзнику, который стоит за вашей спиной.
* На [Цепное исцеление](https://ru.wowhead.com/spell=1064) оказывают влияние все характеристики, кроме Искусности.
* Снимает по стаку [Мучительного](https://ru.wowhead.com/affix=12) аффикса с каждого отхиленного союзника.
* [Цепное исцеление](https://ru.wowhead.com/spell=1064) используется гораздо реже, чем [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004), так как дольше кастуется и требует нахождения союзников в радиусе **15** метров друг от друга.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=51505" target="blank" data-wh-icon-size="small" > **Выброс лавы**</a> – основное заклинание с коротким КД, генерирует большое количество энергии Водоворота и всегда наносит критический удар, если на цели есть [Огненный шок](https://ru.wowhead.com/spell=188389).
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Lava Burst, LvB, Meatball, Лвб, Лава бурст, Лава (одно из значений).  

* Данный способность доступна Элем Шаману по умолчанию, без траты очков талантов.
* Время восстановления **8** секунд.
* **Всегда** наносит критический удар по цели, на которую наложен {{ site.data.spells.fs }}, за счет пассивной способности [Выброс лавы: Ранг 2](https://ru.wowhead.com/spell=231721/).
* Время произнесения – **2** секунды.
* Наносит урон от магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и стоит **2.5%** маны.
* Генерирует **10** ед. энергии Водоворота и дополнительно **4** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534). С талантом [Поток силы](https://www.wowhead.com/ru/spell=385923) генерирует **12** и **5** ед. энергии Водоворота соответственно.
* {{ site.data.spells.lvb }} имеет время полета снаряда около **1** секунды для максимальной дистанции.
* Проверка на наличие [Огненного шока](https://ru.wowhead.com/spell=188389) на цели происходит в момент попадания снаряда, а не в момент завершения каста [Выброса лавы](https://ru.wowhead.com/spell=51505).
* Если сразу после окончания каста [Выброса лавы](https://ru.wowhead.com/spell=51505) использовать {{ site.data.spells.fs }}, то это гарантирует критический удар.
* Обратная ситуация – если во время полета [Выброса лавы](https://ru.wowhead.com/spell=51505) закончится {{ site.data.spells.fs }}, то критический удар не гарантируется.
* Критический удар **НЕ ВЛИЯЕТ** на урон [Выброса лавы](https://ru.wowhead.com/spell=51505) (не считая активного таланта [Перерождение](https://ru.wowhead.com/spell=114050)).
* Усиливается следующими талантами:
  * [Лед и пламя](https://ru.wowhead.com/spell=382886) –  **3%** урона.
  * [Волна лавы](https://ru.wowhead.com/spell=77756) – каждый тик [Огненного шока](https://ru.wowhead.com/spell=188389) имеет **10%** шанс восстановить [Выброс лавы](https://ru.wowhead.com/spell=51505) и сделать его мгновенным.
  * [Зов огня](https://ru.wowhead.com/spell=378255) – добавляет **10%** урона.
  * [Улучшенное оружие языка пламени](https://ru.wowhead.com/spell=382027) – добавляет **5%** урона. 
  * [Камера эхо](https://www.wowhead.com/ru/spell=382032/) – увеличивает урон от прока Искусности на **8%/15%**.
  * [Поток силы](https://www.wowhead.com/ru/spell=385923) – увеличивает генерацию энергии Водоворота.
  * [Эхо стихий](https://www.wowhead.com/ru/spell=333919) – добавляет второй заряд.
  * [Расплавленная порода](https://www.wowhead.com/ru/spell=381776/) – Увеличивает урон на **20%**.
  * [Прилив мощи](https://www.wowhead.com/ru/spell=262303/) – сокращает время восстановления [Элементаля огня](https://ru.wowhead.com/spell=198067) и [Элементаля бури](https://ru.wowhead.com/spell=192249) на **6** секунд.
  * [Извержение говорящего с ветром](https://www.wowhead.com/ru/spell=378268) – увеличивает урон на **10%** и дает гарантированный прок [Волны лавы](https://ru.wowhead.com/spell=77756).
  * [Перерождение](https://www.wowhead.com/ru/spell=114050) – во время действия баффа пропадает кулдаун и урон увеличивается на значение шанса Критического удара.
  * [Первозданная волна](https://www.wowhead.com/ru/spell=375982) – позволяет поразить всех противников, на ком действует [Огненные шоки](https://ru.wowhead.com/spell=188389).
  * [Волна жара](https://www.wowhead.com/ru/spell=386474) – добавляет **20%** урона.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=108271" target="blank" data-wh-icon-size="small" > **Астральный сдвиг**</a> – основное защитное заклинание. Снижает получаемый урон на **40%** на **8** секунд с кулдауном **2** минуты. 
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Astral Shift, AS, Shift, АС, Шифт, Астрал, Сейв, Личка, Минус сорок.

* Не имеет ГКД.
* **Не может** применяться во время оглушения, заморозки, паралича, страха и сна.
* Эффекты снижения получаемого урона складываются **мультипликативно**, то есть значения снижения урона перемножаются, а не складываются.
* Может быть усилен одним из связанных талантов – [Между мирами](https://www.wowhead.com/ru/spell=381647) и [Астральный бастион](https://www.wowhead.com/ru/spell=377933).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=188443" target="blank" data-wh-icon-size="small" > **Цепная молния**</a> – наносит ощутимый урон **5** целям, основной генератор энергии Водоворота на АоЕ.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Chain Lightning, CL, КЛ (одно из значений), Чайник (одно из значений), Цепнуха (одно из значений).  

* Данная способность доступна Элем Шаману по умолчанию, без траты очков талантов.
* Время произнесения – **2** секунды. Талант [Безжалостное бедствие](https://www.wowhead.com/ru/spell=382685) сокращает время произнесения до **1.75** секунд.
* Наносит урон от сил <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1%** маны.
* Генерирует по **4** ед. энергии Водоворота за каждую пораженную цель и дополнительно по **3** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* Эффективность [Искусности](https://ru.wowhead.com/spell=168534) для [Цепной молнии](https://ru.wowhead.com/spell=188443) **снижена до 33%**. То есть, если у вас **60%** Искусности, то [Цепная молния](https://ru.wowhead.com/spell=188443) имеет только **20%** на прок перегрузки.
* [Искусность](https://ru.wowhead.com/spell=168534) и шанс Критического удара просчитывается на каждую отдельную цель, вне зависимости от остальных.
* {{ site.data.spells.cl }} не имеет времени полета снаряда (потому что [очень быстро](https://ru.wikipedia.org/wiki/%D0%9C%D0%BE%D0%BB%D0%BD%D0%B8%D1%8F) двигается).
* Радиус перескока [Цепной молнии](https://ru.wowhead.com/spell=188443) составляет **5** метров и считается от края хитбокса цели, а не от центра.
* Логика перескока работает таким образом, чтобы задеть максимальное количество целей. Если вы кастуете [Цепную молнию](https://ru.wowhead.com/spell=188443) в группу разрозненных врагов, то она будет двигаться в ту стороне, где сможет задеть больше противников.
* Под действием таланта [Перерождение](https://ru.wowhead.com/spell=114050) {{ site.data.spells.cl }} заменяется на альтернативную, «огненную» версию – [Поток лавы](https://ru.wowhead.com/spell=114074).
* Усиливается следующими талантами:
  * [Гнев природы](https://www.wowhead.com/ru/spell=381655) – повышает на **2/4%** шанс Критического удара.
  * [Хранитель бурь](https://www.wowhead.com/ru/spell=191634) – позволяет применять мгновенно и гарантирует срабатывания [Перегрузки](https://ru.wowhead.com/spell=168534) на каждой цели.
  * [Призыв грома](https://www.wowhead.com/ru/spell=378241/) – добавляет **15%** урона.
  * [Безжалостное бедствие](https://www.wowhead.com/ru/spell=382685) – сокращает время применения на **0.25** секунд.
  * [Вспышка молнии](https://www.wowhead.com/ru/spell=381936) – дает снижение кулдауна природных заклинаний за каждый каст.
  * [Электрический шок](https://www.wowhead.com/ru/spell=382086) – добавляет **15%** урона.
  * [Грозовой разрядник](https://www.wowhead.com/ru/spell=210689/) – добавляет **20%** урона за каждый наложенный дебафф.
  * [Властелин стихий](https://www.wowhead.com/ru/spell=16166/) – добавляет **20%** урона.
  * [Прилив мощи](https://www.wowhead.com/ru/spell=262303/) – позволяет поразить **6 целей.
  * [Мощь Водоворота](https://www.wowhead.com/ru/spell=191861) – добавляет **1** гарантированное срабатывание [Перегрузки](https://ru.wowhead.com/spell=168534).
  * [Камера эхо](https://www.wowhead.com/ru/spell=382032) – увеличивает урон от [Перегрузки](https://ru.wowhead.com/spell=168534) на **15%**.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


## Второй ряд

<a href="https://ru.wowhead.com/spell=198103" target="blank" data-wh-icon-size="small" > **Элементаль земли**</a> – призыв защитного элементаля, который забирает на себя ближайших противников и увеличивает запас здоровья игрока на **15%** на **60** секунд.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Earth Elemental, Earth, Steve, Элем земли, Игорь, Стив. 
* При активации способности призывается Элементаль, который атакует **одного** противника в ближнем бою, нанося **25%** урона от силы заклинаний раз в **2** секунды. Тип урона – <span style="color:#FFFFFF;font-size:1em;">**Физический**</span>.
* Увеличивает запас здоровья игрока на **15%** на время своего действия.
* Длительность [Элементаля земли](https://ru.wowhead.com/spell=198103) составляет **60** секунд.
* Время восстановления – **5** минут.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) призывается мгновенно, считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Имеет агро-ауру вокруг себя, благодаря которой может переагрить мобов даже с танка.
* Помимо этого, Элементаль также имеет АоЕ таунт, который срабатывает при следующих условиях:
  1. Любой **не-танк** получает агро.
  2. Танк отошел на **20** метров от заагренной цели.
  3. Здоровье танка опустилось ниже **35%**.
* **Не таунтит боссов**, но может забрать на себя его прислужников. На боях, где необходимо точное позиционирование мобов, его лучше не призывать.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) получает усиления от всех характеристик (Интеллект, Критический удар, Скорость, Универсальность), кроме Искусности.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) имеет **150%** здоровья и **600%** брони шамана, а шанс критического удара мобов по нему снижен на **6%**.
* Сила [Элементаля земли](https://ru.wowhead.com/spell=198103) меняется динамически от ваших статов, то есть не имеет значения, когда использовать [Жажду крови](https://ru.wowhead.com/spell=2825) или прожимные аксессуары, до или после призыва.
* Однако его здоровье снапшотится в момент призыва. Чем больше у вас максимального ХП – тем толще [Элементаль земли](https://ru.wowhead.com/spell=198103).
* Усиленный талантом {{ site.data.spells.primal_elem }}, {{ site.data.spells.igor }} увеличивает свою атаку на **80%** и получает следующие способности:
  * [Разгневанный дух земли](https://ru.wowhead.com/spell=36213) – активная способность, генерирует большое количество угрозы раз в **5** секунд в радиусе **15** метров. Применяется элементалем автоматически.
  * [Раздавить](https://ru.wowhead.com/spell=118345/) – оглушение одной цели на **4** секунды. Применяется автоматически сразу после вызова [Элементаля земли](https://ru.wowhead.com/spell=198103). Имеет время восстановления **40** секунд, то есть применяется два раза за один призыв элема.
* С талантом {{ site.data.spells.primal_elem }} {{ site.data.spells.igor }} считается **«питомцем»**, а без него – **«стражем»**.
* С этим талантом невозможно призвать одновременно [Элементаля земли](https://ru.wowhead.com/spell=198103) и [Элементаля огня](https://ru.wowhead.com/spell=198067), так как последний призванный элементаль заменяет предыдущего. Не может быть двух питомцев одновременно, а стражей – сколько угодно.
* Также с этим талантом появляется возможность управлять [Элементалем земли](https://ru.wowhead.com/spell=198103), например с помощью команды «Отправить» вы можете указать, куда ему следует подойти.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=57994" target="blank" data-wh-icon-size="small" > **Пронизывающий ветер**</a> – прерывание заклинания противника.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Wind Shear, WS, Интерапт, Сбивалка, Ветер, Кик.

* Не имеет ГКД.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **12** секунд, **самый короткий КД интерапта в игре**.
* Радиус действия [Пронизывающего ветра](https://ru.wowhead.com/spell=57994) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* После сбивания не дает противнику кастовать заклинание той же школы магии в течение **3** секунд.
* При использовании [Пронизывающего ветра](https://ru.wowhead.com/spell=57994) во время произнесения другого заклинания оно будет прервано (подобно команде /stopcasting).
* Усиливается талантом [Затопление](https://ru.wowhead.com/spell=378776), давая **8** единиц энергии Водоворота при успешном прерывании заклинания.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=381647" target="blank" data-wh-icon-size="small" > **Между мирами**</a> – усиливает [Астральный сдвиг](https://ru.wowhead.com/spell=108271), снижая его кулдаун до полутора минут и продлевая время действия до **12** секунд.  

* <a href="https://ru.wowhead.com/spell=377933" target="blank" data-wh-icon-size="small" > **Астральный бастион**</a> – усиливает [Астральный сдвиг](https://ru.wowhead.com/spell=108271), увеличивая размер снижения урона до **55%**.  

<hr style="height:1px;background-color:#bbb">
<p></p>


Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=381647" target="blank" data-wh-icon-size="small" > **Дух волка**</a> – усиливает [Призрачного волка](https://ru.wowhead.com/spell=2645), дополнительно повышаая скорость передвижения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645) на **20%**, а также дает снижение получаемого урона на **20%**. Эффект накапливается по **5%** за одну секунду нахождения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645).  
 
* <a href="https://ru.wowhead.com/spell=378075" target="blank" data-wh-icon-size="small" > **Громоподобные лапы**</a> – усиливает [Призрачного волка](https://ru.wowhead.com/spell=2645), позволяя раз в **60** секунд при входе в форму [Призрачного волка](https://ru.wowhead.com/spell=2645) снять с себя все замедления и увеличить скорость передвижения на **25%** на **3** секунды.  

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=196840" target="blank" data-wh-icon-size="small" > **Ледяной шок**</a> – мгновенное заклинание, наносящее небольшой урон и замедляющее цель. Необходимый талант при взятой [Ледяной ярости](https://ru.wowhead.com/spell=210714) в спековой ветке.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Frost Shock, FrS, Фрост шок, Фрост.  

* Кастуется мгновенно и не имеет времени восстановления.
* Наносит урон от магии <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> и стоит **1%** маны.
* [Искусность](https://ru.wowhead.com/spell=168534) никак не взаимодействует с [Ледяным шоком](https://ru.wowhead.com/spell=196840).
* Не имеет времени полета снаряда, то есть накладывается мгновенно.
* Снижает скорость передвижения цели на **50%** на **6** секунд.
* С талантом [Ледяная ярость](https://ru.wowhead.com/spell=210714), {{ site.data.spells.frs }} превращается в очень мощное заклинание, нанося в **4** раза больше урона и генерируя **8** ед. энергии Водоворота.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=187880" target="blank" data-wh-icon-size="small" > **Оружие Водоворота**</a> – атаки в ближнем бою имеют шанс повысить скорость произнесения следующего заклинания на **20%**. 

<hr style="height:1px;background-color:#bbb">
<p></p>


## Третий ряд

<a href="https://ru.wowhead.com/spell=974" target="blank" data-wh-icon-size="small" > **Щит земли**</a> – стихийный щит, увеличивающий получаемое от вас исцеление и лечащий при получении урона.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Earth Shield, ES, Земляной щит, Ерсшилд.

* Вешает бафф на **10** минут, который можно применить к игроку или союзнику.
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Возможно поддерживать только один щит стихий одновременно – либо [Щит молний](https://ru.wowhead.com/spell=192106), либо [Щит земли](https://ru.wowhead.com/spell=974). Талант [Кружащие стихии](https://www.wowhead.com/ru/spell=383010) снимает это ограничение.
* Талант [Ободряющие щиты](https://www.wowhead.com/ru/spell=382033/) увеличивает лечение на **13%/25%
* [Щит земли](https://ru.wowhead.com/spell=974) усиливает все исцеление игрока, наложившего бафф, на **20%**.
* Бафф на увеличение исцеления действует только для того шамана, кто наложил {{ site.data.spells.eshield }}.
* Исцеление аксессуаров **не усиляется** на **20%**.
* Увеличение отхила работает как «белый список», только на способности шамана. Полный список есть на странице заклинания на Wowhead.
* Помимо этого, каждый раз, когда цель с [Щитом земли](https://ru.wowhead.com/spell=974) получает урон, она хилится на **73.5%** от силы заклинаний. 
* Имеет **9** зарядов на срабатывание отхила, после чего [Щит земли](https://ru.wowhead.com/spell=974) спадает и его надо обновлять.
* Внутренний кулдаун на лечение при получении урона – **3** секунды.
* {{ site.data.spells.eshield }} от другого шамана никак не взаимодействует с вашим. Они могут срабатывать как вместе, так и по отдельности.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=382886" target="blank" data-wh-icon-size="small" > **Лед и пламя**</a> – пассивный талант, увеличивает урон от заклинаний школ магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> на **3%**. Действует на [Выброса лавы](https://ru.wowhead.com/spell=51505), [Огненный шок](https://ru.wowhead.com/spell=188389), [Ледяной шок](https://ru.wowhead.com/spell=196840) и [Ледяную ярость](https://ru.wowhead.com/spell=210714).

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=192058" target="blank" data-wh-icon-size="small" > **Тотем конденсации**</a> – АоЕ оглушение врагов на **3** секунды, срабатывающее через **2** секунды после установки тотема. 
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
* При использовании устанавливается тотем в выбранном месте, оглушающий ближайших врагов. Оглушение срабатывает не сразу, а спустя **2** секунды после установки тотема.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Длительность оглушения – **3** секунды.
* Радиус действия – **8** метров вокруг тотема.
* Радиус установки тотема – **40** метров вокруг персонажа.
* Время восстановления – **1** минута.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Может быть усилен одним из связанных талантов – [Статический разряд](https://www.wowhead.com/ru/spell=265046) и [Дубина стража](https://www.wowhead.com/ru/spell=381819).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=51886" target="blank" data-wh-icon-size="small" > **Очищение духа**</a> – снятие проклятия с себя или союзника. 
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Cleanse Spirit, Decurse, Очищение, Декурс, Диспел.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **6.5%** маны.
* Время восстановления составляет **8** секунд, но только в том случае, если было снято проклятие, иначе потратится только ГКД и мана.
* [Возрождение духов](https://ru.wowhead.com/spell=77130), аналог [Очищения духов](https://ru.wowhead.com/spell=51886) у Рестор шамана, снимает не только проклятия, но и магию с союзников.
* Аналоги [Очищения духов](https://ru.wowhead.com/spell=51886) у других классов:
  * [Снятие порчи](https://ru.wowhead.com/spell=2782) **Друидов**.
  * [Снятие проклятия](https://ru.wowhead.com/spell=475) **Магов**.
  * [Прижигающее пламя](https://www.wowhead.com/ru/spell=374251) **Пробудителей**.
  * [Каменная форма](https://ru.wowhead.com/spell=20594) **Дворфов** (только с себя).
  * [Огненная кровь](https://ru.wowhead.com/spell=265221) **Дворфов из клана Черного Железа** (только с себя).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=381666" target="blank" data-wh-icon-size="small" > **Точное озарение**</a> – после применения [Огненного шока](https://ru.wowhead.com/spell=188389) увеличивает лечение следующего заклинания на **15%/30%** и снижает его стоимость маны на **10%/20%** (в зависимости от ранга).

<hr style="height:1px;background-color:#bbb">
<p></p>

## Четвертый ряд

<a href="https://ru.wowhead.com/spell=383010" target="blank" data-wh-icon-size="small" > **Кружащие стихии**</a> – позволяет держать на себе сразу два стихийных щита ([Щит молний](https://ru.wowhead.com/spell=192106) и [Щит земли](https://ru.wowhead.com/spell=974)), а также применять [Щит земли](https://ru.wowhead.com/spell=974) на себя и на союзника.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=79206" target="blank" data-wh-icon-size="small" > **Благосклонность предков**</a> – позволяет применять заклинания на ходу в течение **15** секунд раз в **2** минуты.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Spiritwalker's Grace, SWG, СВГ, Спиритволк, Каст на ходу.  

* Не имеет ГКД.
* Может применяться во время произнесения другого заклинания.
* [Благосклонность предков](https://ru.wowhead.com/spell=79206) считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **14%** маны.
* Работает как «белый список», то есть действует только на те способности и заклинания, которые указаны на странице заклинания на Wowhead.
* Можно усилить одним из связанных талантов – [Благосклонный дух](https://www.wowhead.com/ru/spell=192088) и [Эгида предков](https://www.wowhead.com/ru/spell=378077).
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=382947" target="blank" data-wh-icon-size="small" > **Защита предков**</a> – пассивный талант, увеличивает Самоисцелени и Избежание на **2%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=8143" target="blank" data-wh-icon-size="small" > **Тотем трепета**</a> – снимает с союзников эффекты страха, подчинения и сна. Можно поставить тотем до применения эффекта, чтобы сразу же снять его в том числе и с себя.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Tremor Totem, Тремор, Антифир. 

* При использовании призывается тотем под ноги персонажа, который снимает с союзников эффекты страха, подчинения и сна.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **2.3%** маны.
* Снимает эффекты раз в **1** секунду, начиная с момента установки.
* Длительность составляет **10** секунд
* Радиус действия – **30** метров вокруг тотема.
* Время восстановления – **1** минута.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=265046" target="blank" data-wh-icon-size="small" > **Статический разряд**</a> – усиливает [Тотем конденсации](https://ru.wowhead.com/spell=192058), снижая его кулдаун на **5** секунд за каждого пораженного противника, вплоть до **20** секунд.  

* <a href="https://ru.wowhead.com/spell=381819" target="blank" data-wh-icon-size="small" > **Дубина стража**</a> – усиливает [Тотем конденсации](https://ru.wowhead.com/spell=192058), призывая его повторно после исчезновения. Второй стан будет длиться короче за счет Diminishing Returns.

<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=370" target="blank" data-wh-icon-size="small" > **Развеивание магии**</a> – снимает с противника один благотворный магический эффект, не имеет кулдауна.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Purge, Пурдж, Диспел (одно из значений), Очищение, Снятие магии.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Тратит ГКД, но не имеет времени восстановления.
* Магические эффекты снимаются только с противников, но не с союзников.
* Магический бафф отображается на противнике как иконка с белой рамкой.
* Настроить отображение можно с помощью [WA](https://wago.io/r2QBcOnlT) или [мода на Plater](https://wago.io/turHC0a5k).
* Радиус действия [Развеивания магии](https://ru.wowhead.com/spell=370) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* Аналоги [Развеивания магии](https://ru.wowhead.com/spell=370) у других классов:
  * [Рассеивание заклинаний](https://ru.wowhead.com/spell=528/) **Жреца** (без КД).
  * [Усмиряющий выстрел](https://ru.wowhead.com/spell=19801) **Охотника** (**10** секунд КД).
  * [Поглощение магии](https://ru.wowhead.com/spell=278326) **Охотника на демонов** (**10** секунд КД).
  * [Чарокрад](https://ru.wowhead.com/spell=30449) **Мага** (ворует заклинание, а не снимает, без КД).
  * [Волшебный поток](https://ru.wowhead.com/spell=28730) **Кровавых эльфов** (**2** минуты КД).
  * [Пожирание магии](https://ru.wowhead.com/spell=19505) питомца **Чернокнижника** (**15** секунд КД).
</details>
<p></p>

* <a href="https://ru.wowhead.com/spell=378773" target="blank" data-wh-icon-size="small" > **Великое очищение**</a> – снимает с противника два благотворных магических эффекта, имеет кулдаун **12** секунд.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=382888" target="blank" data-wh-icon-size="small" > **Шквал**</a> – нанесение критического удара увеличивает скорость атаки в ближнем бою.

<hr style="height:1px;background-color:#bbb">
<p></p>

## Пятый ряд

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=192088" target="blank" data-wh-icon-size="small" > **Благосклонный дух**</a> – усиливает [Благосклонность предков](https://ru.wowhead.com/spell=79206), снижая его кулдаун на **30** секунд и увеличивая скорость передвижения во время его действия на **20%**.

* <a href="https://ru.wowhead.com/spell=378077" target="blank" data-wh-icon-size="small" > **Эгида предков**</a> – усиливает [Благосклонность предков](https://ru.wowhead.com/spell=79206), давая иммунитет к прерываниям заклинаний на **5** секунд после его применения.

<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=192077" target="blank" data-wh-icon-size="small" > **Тотем ветряного порыва**</a> – установка тотема, увеличивающего скорость передвижения союзников на **40%** с кулдаун **2** минуты.

<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Wind Rush Totem, WRT, ВРТ, Хаст тотем, Тотем ветроступа, Тотем на скорость.

* При использовании устанавливается тотем в выбранном месте, увеличивающий скорость передвижения союзников на **40%**.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **2** минуты. 
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Длительность действия баффа на скорость передвижения – **5** секунд.
* Бафф обновляется каждую секунду, пока вы находитесь в области действия тотема.
* Радиус действия – **10** метров.
* Сам тотем существует **15** секунд.
* Можно установить в любую точку на расстоянии **40** метров от персонажа.
* Для быстрой установки [Тотема ветряного порыва](https://ru.wowhead.com/spell=192077) под персонажа можно использовать следующий макрос:
~~~
#showtooltip
/cast [@player] Тотем ветряного порыва
~~~
* Усиливается следующими талантами:
  * [Сила тотемов](https://www.wowhead.com/ru/spell=381867/) – сокращает кулдаун на **2/4** секунды.
  * [Проецирование тотемов](https://www.wowhead.com/ru/spell=108287) – переносит все установленные тотемы в указанное место.
  * [Тотемное средоточие](https://www.wowhead.com/ru/spell=382201) – увеличивает радиус и время действия тотема.
  * [Возвращение тотемов](https://www.wowhead.com/ru/spell=108285) – сбрасывает кулдаун последнего примененного тотема.
</details>
<p></p>

* <a href="https://ru.wowhead.com/spell=51485" target="blank" data-wh-icon-size="small" > **Тотем хватки земли**</a> – установка тотема, обездвиживащего врагов в указанной области. Урон прерывает действие эффекта.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Earthgrab Totem, EGT, Тотем рутов.

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **0.5%** маны.
* Обездвиживает цели вокруг тотема на **8** секунд.
* Не работает на боссов.
* Радиус действия – **10** метров вокруг тотема.
* Эффект от тотема накладывается раз в **2** секунды.
* Длительность жизни тотема – **20** секунд
* Время восстановления – **30** секунд.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Усиливается следующими талантами:
  * [Сила тотемов](https://www.wowhead.com/ru/spell=381867/) – сокращает кулдаун на **2/4** секунды.
  * [Проецирование тотемов](https://www.wowhead.com/ru/spell=108287) – переносит все установленные тотемы в указанное место, обездвиживая новую группу врагов.
  * [Тотемное средоточие](https://www.wowhead.com/ru/spell=382201) – увеличивает радиус и время действия тотема.
  * [Возвращение тотемов](https://www.wowhead.com/ru/spell=108285) – сбрасывает кулдаун последнего примененного тотема.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=51514" target="blank" data-wh-icon-size="small" > **Сглаз**</a> – превращает гуманоида или животное в зверушку, не давая возможности атаковать в течении **1** минуты.

<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Hex, Хекс, Лягушка, Контроль, Полиморф шамана.  

* При использовании превращает цель в зверька, не позволяя совершать какие-либо действия, кроме медленного передвижения.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Превращает цель в лягушку (или другую форму) на **60** секунд, не позволяя совершать какие-либо действия.
* В PvP базовое время действия составляет **8** секунд, без учета Diminishing returns.
* Является Проклятием, а не Магией.
* Время восстановления **30** секунд.
* Время произнесения – **1.7** секунд.
* Радиус действия [Сглаза](https://ru.wowhead.com/spell=51514) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* Работает только на **гуманоидов** и **зверей**.
* Не сбивается уроном мгновенно, в отличие от других подобных заклинаний.
* При использовании [Сглаза](https://ru.wowhead.com/spell=51514) вне боя не вводит в комбат, как и другие эффекты долгого контроля.
* Не уменьшает агро-радиус цели, в отличие от [Пленения](https://ru.wowhead.com/spell=217832) Охотника на демонов и [Ошеломления](https://ru.wowhead.com/spell=6770) Разбойника.
* Имеет множество различных форм, способы открытия есть по ссылке на Wowhead:
  * [Лягушка](https://ru.wowhead.com/spell=51514)
  * [Компсогнат](https://ru.wowhead.com/spell=210873)
  * [Змея](https://ru.wowhead.com/spell=211010)     
  * [Детеныш-скелет](https://ru.wowhead.com/spell=269352)
  * [Плетеный кусака](https://ru.wowhead.com/spell=277784)
  * [Сгусток живого меда](https://ru.wowhead.com/spell=309328)
  * [Зандаларский жилорез](https://ru.wowhead.com/spell=277778)
  * [Таракан](https://ru.wowhead.com/spell=211015)
  * [Паук](https://ru.wowhead.com/spell=211004)          
* Усиливается следующими талантами:
  * [Сильное вуду](https://ru.wowhead.com/spell=204268) – снижает кулдаун до **15** секунд.
  * [Ослабление](https://ru.wowhead.com/spell=378079) – по окончанию действия замедляет цель на **70%** на **4** секунды.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=381655" target="blank" data-wh-icon-size="small" > **Гнев природы**</a> – пассивный талант, увеличивает шанс критического удара заклинаний пророды на **2%/4%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

## Шестой ряд

<a href="https://ru.wowhead.com/spell=381867" target="blank" data-wh-icon-size="small" > **Сила тотемов**</a> – пассивный талант, сокращает время восстановления тотемов на **2/4** секунды.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=381650" target="blank" data-wh-icon-size="small" > **Защита от стихий**</a> – пассивный талант, снижает получаемый магический урон на **2%/4%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=30884" target="blank" data-wh-icon-size="small" > **Cтраж природы**</a> – пассивный талант, исцеляет вас на **10%/20%**, когда уровень вашего здоровья падает ниже **35%** от максимального запаса. Срабатывает не чаще раза в **45** секунд.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Nature’s Guadrian, NG, Страж, прок хила.

* Этот талант может спасти от смерти в том случае, если количество избыточного урона не превысило количество его исцеления.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=204268" target="blank" data-wh-icon-size="small" > **Сильное вуду**</a> – усиливает [Сглаз](https://ru.wowhead.com/spell=51514), сокращая его кулдаун до **15** секунд.

* <a href="https://ru.wowhead.com/spell=378079" target="blank" data-wh-icon-size="small" > **Ослабление**</a> – усиливает [Сглаз](https://ru.wowhead.com/spell=51514), замедляя цель на **70%** на **4** секунды по окончанию его действия.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=382215" target="blank" data-wh-icon-size="small" > **Ветра Ал'акира**</a> – пассивный талант, увеличивает скорость передвиженияв форме [Призрачного волка](https://www.wowhead.com/ru/spell=2645) на **5%/10%**. Также, при трех активных тотемах, увеличивает передвижения на **7%/15%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=381689" target="blank" data-wh-icon-size="small" > **Переполняющая сила жизни**</a> – пассивный талант, увеличивает максимальный запас здоровья на **8%** если [Реинкарнация](https://ru.wowhead.com/spell=20608) не на кулдауне, а при полном здоровье кулдаун [Реинкарнации](https://ru.wowhead.com/spell=20608) сокращается до **~17** минут.

<hr style="height:1px;background-color:#bbb">
<p></p>

## Седьмой ряд

<a href="https://ru.wowhead.com/spell=5394" target="blank" data-wh-icon-size="small" > **Тотем исцеляющего потока**</a> – тотем, который дает небольшой пассивный отхил в течение **15** секунд.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Healing Stream Totem, HST, ХСТ, Тотемчик, Хил-тотем, Малыш.

* При использовании призывается тотем под ноги персонажа, который действует **15** секунд и раз в **2** секунды лечит одного союзника.
* Время восстановления – **30** секунд. 
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **9%** маны.
* За один тик хилит на **47%** от силы заклинаний, то есть суммарно **352.5%** от силы заклинаний за все время действия тотема.
* Скорость добавляет дополнительный тик в конце действия эффекта, ослабленный в зависимости от недостающего времени до «полного» тика.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* На каждом тике выбирает случайную раненую цель в радиусе действия тотема, **40** метров.
* Использование в бою [Тотем исцеляющего потока](https://ru.wowhead.com/spell=5394) ведет к снижению урона, так как вы тратите ГКД на его установку.
* Используйте [Тотем исцеляющего потока](https://ru.wowhead.com/spell=5394) в те моменты, когда невозможно атаковать, либо перед или после боя.
* Усиливается следующими талантами:
  * [Сила тотемов](https://www.wowhead.com/ru/spell=381867/) – сокращает кулдаун на **2/4** секунды.
  * [Проецирование тотемов](https://www.wowhead.com/ru/spell=108287) – переносит все установленные тотемы в указанное место, обездвиживая новую группу врагов.
  * [Тотемное средоточие](https://www.wowhead.com/ru/spell=382201) – увеличивает радиус и время действия тотема.
  * [Возвращение тотемов](https://www.wowhead.com/ru/spell=108285) – сбрасывает кулдаун последнего примененного тотема.
  * [Кружашие потоки](https://www.wowhead.com/ru/spell=378094) – установка тотема усиливает **3** следующие заклинания исцеления на **13%/25%**.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


<a href="https://ru.wowhead.com/spell=381674" target="blank" data-wh-icon-size="small" > **Улучшенная молния**</a> – пассивный талант, увеличивает урон [Молнии](https://ru.wowhead.com/spell=188196) на **20%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

<a href="https://ru.wowhead.com/spell=108287" target="blank" data-wh-icon-size="small" > **Проецирование тотемов**</a> – переносит все установленные тотемы в указанное место.

<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>

* Время восстановления – **10** секунд.
* Не имеет ГКД.
* Имеет несколько вариантов применения:
  * Позволяет перенести [Тотем жидкой магмы](https://www.wowhead.com/ru/spell=192222/) в том случае, если враги вышли из области его действия.
  * С помощью переноса [Тотема хватки земли](https://www.wowhead.com/ru/spell=51485) можно обездвижить вторую группу врагов.
  * Перенос [Тотема ветряного порыва](https://www.wowhead.com/ru/spell=192077) позволит продлить ускорение.
  * Убирает необходимость подходить в зону ближнего боя для установки [Тотема каменной кожи](https://www.wowhead.com/ru/spell=383017) для баффа танков.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>


Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=58875" target="blank" data-wh-icon-size="small" > **Поступь духа**</a> – увеличивает скорость передвижения на **60%** на **8** секунд, эффекты обездвиживания в момент активации. Кулдаун – **60** секунд.

* <a href="https://ru.wowhead.com/spell=192063" target="blank" data-wh-icon-size="small" > **Порыв ветра**</a> – рывок на среднее расстояние с кулдауном **30** секунд. Позволяет избежать урона от падения.

<hr style="height:1px;background-color:#bbb">
<p></p>


## Восьмой ряд

* <a href="https://ru.wowhead.com/spell=378094" target="blank" data-wh-icon-size="small" > **Кружащие потоки**</a> – после применения [Тотема исцеляющего потока](https://www.wowhead.com/ru/spell=5394) три ваших следующих лечащих заклинаний будут сильнее на **13%/25%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=378081" target="blank" data-wh-icon-size="small" > **Природная стремительность**</a> – делает ваше следующее заклинание школы магии природы мгновенным. Кулдаун заклинания – **60** секунд и оно не имеет ГКД.

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=51490" target="blank" data-wh-icon-size="small" > **Гром и молния**</a> – отбрасывает противников в сторону, после чего замедляет на **40%** на **5** секунд. 
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Thunderstorm, Knockback, Нокбек, Тандер, Откидывание, Отброс.  

* При использовании отбрасывает противников в стороны от персонажа.
* Наносит **10%** урона от силы заклинаний.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **30** секунд.
* При использовании откидывает противников на расстоянии **10** метров от персонажа на **20** метров.
* После откидывания замедляет пораженных противников на **40%** на **5** секунд.
* Можно усилить талантом [Громовой удар](https://www.wowhead.com/ru/spell=378779), в результате чего [Гром и молния](https://ru.wowhead.com/spell=51490) будет подбрасывать противников вверх и иметь сниженный на **5** секунд кулдаун.
* Можно применять во время оглушения.
* Крайне полезное заклинание в Мифик+ на [Кровавом](https://ru.wowhead.com/affix=8) и [Злопамятном](https://ru.wowhead.com/affix=123) аффиксах.
* Также может пригодиться, если на вас сагрились мобы, с помощью способности их можно откинуть обратно к танку.
* Можно использовать для прерывания заклинаний, в том числе для тех, которые нельзя сбить обычным способом.
* Не работает на боссов.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=382201" target="blank" data-wh-icon-size="small" > **Тотемное средоточие**</a> – увеличивает радиус действия тотемов на **8%/15%** и продлевает их время действия.

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=382033" target="blank" data-wh-icon-size="small" > **Ободняющие щиты**</a> – усиливает эффекты [Щита молний](https://ru.wowhead.com/spell=192106) и [Щита земли](https://ru.wowhead.com/spell=974).

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=381678" target="blank" data-wh-icon-size="small" > **Плыви по течению**</a> – снижает время восстановления [Поступи духа](https://ru.wowhead.com/spell=58875) на **7.5/15** секунд и [Порывв ветра](https://ru.wowhead.com/spell=192063) на **5/10** секунд.

<hr style="height:1px;background-color:#bbb">
<p></p>

## Девятый ряд


* <a href="https://ru.wowhead.com/spell=381930" target="blank" data-wh-icon-size="small" > **Тотем источника маны**</a> – тотем длительностью **2** минуты, который восстанавливает **200** единиц маны четырем союзникам в радиусе **30** метров каждый раз когда вы применяете [Выброс лавы](https://ru.wowhead.com/spell=51505). В первую очередь восстанавливает ману целителям. Увеличивает генерацию маны хилам примерно на **4-7%**.

<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=378779" target="blank" data-wh-icon-size="small" > **Громовой удар**</a> – усиливает [Гром и молнии](https://ru.wowhead.com/spell=51490), заменяя отбрасывание противников в стороны на подбрасывание вверх, и снижает кулдаун на **5** секунд. Полезно в том случае, если нужно сбить применение заклинаний без перемещения противников.

* <a href="https://ru.wowhead.com/spell=305483" target="blank" data-wh-icon-size="small" > **Молния-лассо**</a> – бывший PvP-талант. Потоковое заклинание, наносит средний урон одной цели и оглушает её на время каста. Не используется для урона, а только как контроль.

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=383013" target="blank" data-wh-icon-size="small" > **Тотем противоядия**</a> – снимает по одному яду с союзников каждые **1.5** секунд в течение **6** секунд.

<hr style="height:1px;background-color:#bbb">
<p></p>

* <a href="https://ru.wowhead.com/spell=108285" target="blank" data-wh-icon-size="small" > **Возвращение тотемов**</a> – сбрасывает время восстановления последнего примененного тотема, кулдаун **3** минуты. Обязательный талант в том случае, если взят [Тотем жидкой магмы](https://www.wowhead.com/ru/spell=192222) в древе талантов специализации. Также может сбрасыать кулдаун вспомогательных тотемов, что будет полезно в определенных ситуациях.

<hr style="height:1px;background-color:#bbb">
<p></p>

## Десятый ряд


<a href="https://ru.wowhead.com/spell=108281" target="blank" data-wh-icon-size="small" >**Наставления предков**</a> – в течение **10** секунд превращает **25%** нанесенного урона и исцеления в отхил для трех человек, выбирая в первую очередь раненые цели. Позволяет практически в соло прохиливать всю группу во время большого АоЕ под бурстами, например на первом боссе в Рубиновых Омутах Жизни.

<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Ancestral Guidance, AG, Наставления, АГ.

* При использовании дается бафф на **10** секунд, при котором вы лечите трех союзников на **25%** от нанесенного урона или исцеления.
* Время восстановления – **2** минуты.
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Применяется мгновенно и не имеет ГКД.
* Имеет хард-кап в **3** цели. 
* Радиус лечения – **40** метров.
* Если цель будет одна, то отхил не увеличится, если больше трёх – лечение получат только три.
* Является «смарт-хилом», то есть хилит только раненые цели.
* Лучше всего [Наставления предков](https://ru.wowhead.com/spell=108281) хилит во время АоЕ, так как там вы наносите больше урона.
</details>
<p></p>

<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=383017" target="blank" data-wh-icon-size="small" >**Тотем каменной кожи**</a> – тотем, уменьшающий получаемый физический урон на **10%** для вас и четырех ближайших союзников. Можно перенести этот тотем к танку с помощью [Проецирования тотемов](https://www.wowhead.com/ru/spell=108287). Время действия составляет **15** секунд, а кулдаун – **30** секунд.

* <a href="https://ru.wowhead.com/spell=383019" target="blank" data-wh-icon-size="small" >**Тотем безветрия**</a> – тотем, предовтращающий задержку применения заклинаний при получении урона и сокращающий время действия эффектов прерывания на **50%** для вас и четырех ближайших союзников. 


<hr style="height:1px;background-color:#bbb">
<p></p>

Один из двух талантов на выбор:
* <a href="https://ru.wowhead.com/spell=383011" target="blank" data-wh-icon-size="small" >**Зов стихий**</a> – усиливает [Возвращение тотемов](https://ru.wowhead.com/spell=108285), снижая его время восстановления до **2** минут.

* <a href="https://ru.wowhead.com/spell=383012" target="blank" data-wh-icon-size="small" >**Ядро созидания**</a> – усиливает [Возвращение тотемов](https://ru.wowhead.com/spell=108285), позволяя сбрасывать кулдаун двух тотемов, а не одного.

<hr style="height:1px;background-color:#bbb">
<p></p>

# Таланты специализации Стихии

## Пепвый ряд

## Второй ряд


## Третий ряд


## Четвертый ряд


## Пятый ряд


## Шестой ряд


## Седьмой ряд


## Восьмой ряд


## Девятый ряд


## Десятый ряд














<a href="https://ru.wowhead.com/spell=210714" target="blank" data-wh-icon-size="small" >**Ледяная ярость**</a> – талант шестого тира, является частью ротации в основном билде против одной цели.
<details markdown=1><summary><i>Подробнее...</i></summary>
<p></p>
> Icefury, IF, ИФ, Айсфьюри.  

* Наносит **82.5%** урона от силы заклинаний.
* Дает бафф на **15** секунд, увеличивающий урон **4** следующих [Ледяных шоков](https://ru.wowhead.com/spell=196840) на **300%**.
* Каждый усиленный [Ледяной шок](https://ru.wowhead.com/spell=196840) генерирует **8** ед. энергии Водоворота.
* Время восстановления **30** секунд.
* Наносит урон от магии <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> и стоит **3%** маны.
* Генерирует **25** ед. энергии Водоворота и дополнительно **12** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* {{ site.data.spells.if }} имеет время полета снаряда около **1** секунды для максимальной дистанции, при этом бафф на урон [Ледяных шоков](https://ru.wowhead.com/spell=196840) накладывается сразу же по завершению каста.
* Урон [Ледяной ярости](https://ru.wowhead.com/spell=210714) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}, однако само заклинание наносит урона даже меньше чем {{ site.data.spells.lb }} и бафф по возможности лучше тратить на усиленный {{ site.data.spells.frs }}.
* Данный талант ощутимо усложняет ротацию в бою против одной цели, особенно для новичков. Альтернатива – {{ site.data.spells.primal_elem }}, дает чуть меньше урона, но гораздо проще в использовании.
* Если вы не можете идеально отыгрывать [Ледяную ярость](https://ru.wowhead.com/spell=210714) (без потерь баффов [Ледяного шока](https://ru.wowhead.com/spell=196840)), то лучше использовать [Повелителя изначальных стихий](https://ru.wowhead.com/spell=117013), это даст больше урона.
* После использования [Ледяной ярости](https://ru.wowhead.com/spell=210714) представьте, что вместо четырех следующих [Молний](https://ru.wowhead.com/spell=188196) вам нужно использовать четыре [Ледяных шока](https://ru.wowhead.com/spell=196840).
* В идеале каждый усиленный {{ site.data.spells.frs }} следует бафать талантом {{ site.data.spells.mote }}. 
* Не придерживайте стаки слишком долго, так как **потерять стак баффа [Ледяной ярости](https://ru.wowhead.com/spell=210714) гораздо хуже, чем потерять бафф от [Властелина стихий](https://ru.wowhead.com/spell=16166).**
* Если с талантом {{ site.data.spells.eote }} у вас есть два заряда [Выброса лавы](https://ru.wowhead.com/spell=51505), то используйте между ними усиленный [Ледяной шок](https://ru.wowhead.com/spell=196840).
</details>
<p></p>









































<a href="https://ru.wowhead.com/spell=188196" target="blank" data-wh-icon-size="small" >**Молния**</a> – основной генератор ресурса и «филлер» в бою против одной цели.

> Lightning Bolt, Bolt, LB, Болт, ЛБ.

* Наносит **95%** урона от силы заклинаний.
* Время произнесения – **2** секунды (только после **8** уровня и открытия пассивной способности [Молния: Ранг 2](https://ru.wowhead.com/spell=318044), до этого **2.5** сек).
* Наносит урон от сил <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1%** маны.
* Генерирует **8** ед. энергии Водоворота и дополнительно **3** ед. при проке [Искусности: волнение стихий](https://ru.wowhead.com/spell=168534).
* {{ site.data.spells.lb }} не имеет времени полета снаряда (потому что [очень быстро](https://ru.wikipedia.org/wiki/%D0%9C%D0%BE%D0%BB%D0%BD%D0%B8%D1%8F) двигается).
* В обычной ситуации используется только в бою против **одной цели**.
* С талантом [Хранитель бурь](https://ru.wowhead.com/spell=191634) превращается в бурст способность – кастуется мгновенно, урон увеличивается на **150%** и вылетает гарантированный прок [Перегрузки](https://ru.wowhead.com/spell=168534). С этим баффом {{ site.data.spells.lb }} применяется в том числе в бою против **2** целей.
* Не считая усиления от [Хранителя бурь](https://ru.wowhead.com/spell=191634), {{ site.data.spells.lb }} является нашим «филлером» и кастуется тогда, когда остальные способности не доступны. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Урон [Молнии](https://ru.wowhead.com/spell=188196) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}.

<a href="https://ru.wowhead.com/spell=188389" target="blank" data-wh-icon-size="small" >**Огненный шок**</a> – основной ДоТ.

> Flame Shock, FS, Флейм шок, ФШ, Фаер шок (не стоит), Шок (одно из значений), Дотка.

* Наносит **19.5%** урона от силы заклинаний при наложении, а также **104.4%** урона от силы заклинаний за **18** секунд.
* Наносит урон от магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и стоит **1.5%** маны.
* Периодический урон наносится раз в **2** секунды, это время сокращается за счет Скорости. За каждый тик наносится **11.6%** урона от силы заклинаний.
* Имеет время восстановления **6** секунд, которое **НЕ** снижается Скоростью, в отличие от Шамана Совершенствования.
* Скорость добавляет дополнительный тик в конце действия эффекта, ослабленный в зависимости от недостающего времени до «полного» тика.
* Является **магическим** эффектом, то есть может быть развеяно противником с помощью способностей [Возрождение духа](https://ru.wowhead.com/spell=77130), [Очищение](https://ru.wowhead.com/spell=527), [Детоксикация](https://ru.wowhead.com/spell=115450) и другими.
* Не имеет времени полета снаряда, то есть накладывается мгновенно.
* Базовая длительность действия – **18** секунд.
* При активном [Элементале огня](https://ru.wowhead.com/spell=198067) длительность [Огненного шока](https://ru.wowhead.com/spell=188389) увеличена **вдвое** и составляет **36** секунд (за счет пассивной способности [Элементаль огня: Ранг 2](https://ru.wowhead.com/spell=343226)). Данный эффект **не работает** с [Элементалем бури](https://ru.wowhead.com/spell=192249).
* Если обновить {{ site.data.spells.fs }}, когда до спадения остается менее **5.4** секунд, то это время действия добавится к новому наложению, продлевая шок вплоть до **23.4** секунд (так называемое [«окно пандемии»](https://wowwiki-archive.fandom.com/wiki/Pandemic)).
* Все время свыше **5.4** секунд на момент обновления [Огненного шока](https://ru.wowhead.com/spell=188389) **не добавится** к новому эффекту.
* При активном [Элементале огня](https://ru.wowhead.com/spell=198067) {{ site.data.spells.fs }} можно обновлять на **10.8** секунде, что даст {{ site.data.spells.fs }} длительностью **46.8** секунд.
* Также при активном [Элементале огня](https://ru.wowhead.com/spell=198067) {{ site.data.spells.fs }} тикает на **25%** чаще, то есть раз в **1.5** секунды (без учета Скорости). Данный эффект **не работает** с [Элементалем бури](https://ru.wowhead.com/spell=192249).
* {{ site.data.spells.fs }} взаимодействует с легендаркой {{ site.data.legend.fire }} – шанс крита тиков увеличивается на **50%**, и каждый критический тик сокращает время восстановления [Элементаля огня](https://ru.wowhead.com/spell=198067) или [Элементаля бури](https://ru.wowhead.com/spell=192249) на **1** секунду.
* Количество целей, на которые мы развешиваем [Огненный шок](https://ru.wowhead.com/spell=188389), меняется в зависимости от выбранной легендарки и талантов. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Каждый тик [Огненного шока](https://ru.wowhead.com/spell=188389) имеет **10%** шанс восстановить {{ site.data.spells.lvb }} и сделать следующее его применение мгновенным за счет прока [Волны лавы](https://ru.wowhead.com/spell=77756).
* Способность <span style="color:#40bf40;font-size:1em;">**Некролордов**</span> {{ site.data.covenants.pw }} позволяет накладывать дополнительный {{ site.data.spells.fs }}. Подробнее про [Первозданную волну](https://ru.wowhead.com/spell=326059) в разделе [Ковенанты и медиумы»](https://stormkeeper.ru/ele/covenants.html#%D0%BF%D0%B5%D1%80%D0%B2%D0%BE%D0%B7%D0%B4%D0%B0%D0%BD%D0%BD%D0%B0%D1%8F-%D0%B2%D0%BE%D0%BB%D0%BD%D0%B0).
* Урон [Огненного шока](https://ru.wowhead.com/spell=188389) **НЕ МОЖЕТ** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}, так как он является заклинанием магии <span style="color:#f16f25;font-size:1em;">**Огня**</span>.

<a href="https://ru.wowhead.com/spell=51505" target="blank" data-wh-icon-size="small" >**Выброс лавы**</a> – основное заклинание с коротким КД и проком.

> Lava Burst, LvB, Meatball, Лвб, Лава бурст, Лава (одно из значений).  

* Время восстановления **8** секунд.
* Наносит **97.2%** урона от силы заклинаний и **243%** урона от силы заклинаний при критическом ударе.
* **Всегда** наносит критический удар по цели, на которую наложен {{ site.data.spells.fs }}, за счет пассивной способности [Выброс лавы: Ранг 2](https://ru.wowhead.com/spell=231721/).
* Время произнесения – **2** секунды.
* Наносит урон от магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и стоит **2.5%** маны.
* Генерирует **10** ед. энергии Водоворота и дополнительно **4** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* Прок [Искусности](https://ru.wowhead.com/spell=168534) вылетает не сразу, а спустя примерно **1** секунду после завершения каста.
* Можно скастовать {{ site.data.spells.lvb }} на **90** энергии Водоворота и сразу использовать {{ site.data.spells.es }}, тогда перегрузка вылетит уже после каста [Земного шока](https://ru.wowhead.com/spell=8042) и не уйдет в оверкап.
* {{ site.data.spells.lvb }} имеет время полета снаряда около **1** секунды для максимальной дистанции.
* Проверка на наличие [Огненного шока](https://ru.wowhead.com/spell=188389) на цели происходит в момент попадания снаряда, а не в момент завершения каста [Выброса лавы](https://ru.wowhead.com/spell=51505).
* Если сразу после окончания каста [Выброса лавы](https://ru.wowhead.com/spell=51505) использовать {{ site.data.spells.fs }}, то это гарантирует критический удар.
* Обратная ситуация – если во время полета [Выброса лавы](https://ru.wowhead.com/spell=51505) закончится {{ site.data.spells.fs }}, то критический удар не гарантируется.
* Критический удар **НЕ ВЛИЯЕТ** на урон [Выброса лавы](https://ru.wowhead.com/spell=51505) (не считая активного таланта [Перерождение](https://ru.wowhead.com/spell=114050)).
* Каждый тик [Огненного шока](https://ru.wowhead.com/spell=188389) имеет **10%** шанс восстановить {{ site.data.spells.lvb }} и сделать следующее его применение мгновенным, за счет прока [Волны лавы](https://ru.wowhead.com/spell=77756).
* {{ site.data.spells.lvb }} используется по кулдауну в бою против **1-2** цели и только по проку в бою против **3** целей. В зависимости от талантов и легендарок приоритет может меняться, подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Талант первого тира [Эхо стихий](https://ru.wowhead.com/spell=333919/) дает [Выбросу лавы](https://ru.wowhead.com/spell=51505) второй заряд, в результате чего время восстановления не теряется при проке [Волны лавы](https://ru.wowhead.com/spell=77756), а также сокращает кулдаун [Выброса лавы](https://ru.wowhead.com/spell=51505) до **8** секунд вместо «**8** секунд + время произнесения + время реакции».
* Талант четвертого тира {{ site.data.spells.mote }} позволяет [Выбросу лавы](https://ru.wowhead.com/spell=51505) увеличивать урон следующего заклинания, наносяшего урон от магии <span style="color:#26d22b;font-size:1em;">**Природы**</span>, <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> или <span style="color:#FFFFFF;font-size:1em;">**Физический**</span> на **20%**. Подробнее в разделе [«Таланты»](https://stormkeeper.ru/ele/talents.html#%D1%87%D0%B5%D1%82%D0%B2%D0%B5%D1%80%D1%82%D1%8B%D0%B9-%D1%82%D0%B8%D1%80-35-%D1%83%D1%80%D0%BE%D0%B2%D0%B5%D0%BD%D1%8C).
* Урон [Выброса лавы](https://ru.wowhead.com/spell=51505) **НЕ МОЖЕТ** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}.
* Легендарка {{ site.data.legend.lava }} дает дополнительный заряд [Выброса лавы](https://ru.wowhead.com/spell=51505) и прок [Волны лавы](https://ru.wowhead.com/spell=77756) после использования [Земного шока](https://ru.wowhead.com/spell=8042), а также увеличивает [Выброса лавы](https://ru.wowhead.com/spell=51505) урон на **20%**.

<a href="https://ru.wowhead.com/spell=8042" target="blank" data-wh-icon-size="small" >**Земной шок**</a> – «спендер» в бою против одной цели.

> Earth Shock, ES (одно из значений), Ерсшок, ЕШ, ЗШ, Шок (одно из значений).

* Требует **60** ед. энергии Водоворота.
* Наносит **273%** урона от силы заклинаний.
* Наносит урон от сил <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* [Искусность](https://ru.wowhead.com/spell=168534) не может прокнуть от [Земного шока](https://ru.wowhead.com/spell=8042), но она увеличивает генерацию энергии Водоворота, что ведет к более частому использованию [Земного шока](https://ru.wowhead.com/spell=8042).
* Не имеет времени полета снаряда, то есть накладывается мгновенно.
* Вызывает срабатывание особого эффекта легендарок {{ site.data.legend.lava }} и {{ site.data.legend.eogs }}.
* Применяется **только** в бою против **одной** цели, за исключением использования легендарки {{ site.data.legend.eogs }} – в этом случае [Земной шок](https://ru.wowhead.com/spell=8042) и [Землетрясение](https://ru.wowhead.com/spell=61882) чередуются при любом количестве целей. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Урон [Земного шока](https://ru.wowhead.com/spell=8042) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}.
* В бою против **одной** цели всегда старайтесь усилить [Земной шок](https://ru.wowhead.com/spell=8042) с помощью таланта {{ site.data.spells.mote }}, не уходя в оверкап энергии Водоворот.
* По сути, у нас есть **«окно»** для использования [Земного шока](https://ru.wowhead.com/spell=8042), между **60** и **100** энергии Водоворота, между которым стоит использовать [Выброс лавы](https://ru.wowhead.com/spell=51505) для баффа [Властелином стихий](https://ru.wowhead.com/spell=16166).

<a href="https://ru.wowhead.com/spell=196840" target="blank" data-wh-icon-size="small" >**Ледяной шок**</a> – «филлер» во время перебежек.

> Frost Shock, FrS, Фрост шок, Фрост.  

* Наносит **45%** урона от силы заклинаний и не имеет времени восстановления.
* Наносит урон от магии <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> и стоит **1%** маны.
* [Искусность](https://ru.wowhead.com/spell=168534) никак не взаимодействует с [Ледяным шоком](https://ru.wowhead.com/spell=196840).
* Не имеет времени полета снаряда, то есть накладывается мгновенно.
* Снижает скорость передвижения цели на **50%** на **6** секунд.
* Полезно для нанесения урона на ходу, если нет необходимости обновлять {{ site.data.spells.fs }}, нету мгновенного [Выброса лавы](https://ru.wowhead.com/spell=51505) и не хватает энергии на [Земной шок](https://ru.wowhead.com/spell=8042), а [Благосклонность предков](https://ru.wowhead.com/spell=79206) на КД. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html#%D1%83%D1%80%D0%BE%D0%BD-%D0%B2-%D0%B4%D0%B2%D0%B8%D0%B6%D0%B5%D0%BD%D0%B8%D0%B8). 
* С талантом шестого тира [Ледяная ярость](https://ru.wowhead.com/spell=210714), {{ site.data.spells.frs }} превращается в очень мощное заклинание, нанося в **4** раза больше урона (**180%** урона от силы заклинаний) и генерируя **8** ед. энергии Водоворота.
* Урон [Ледяного шока](https://ru.wowhead.com/spell=196840) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}, но это стоит делать только с активным баффом от таланта [Ледяная ярость](https://ru.wowhead.com/spell=210714).


<a href="https://ru.wowhead.com/spell=210714" target="blank" data-wh-icon-size="small" >**Ледяная ярость**</a> – талант шестого тира, является частью ротации в основном билде против одной цели.

> Icefury, IF, ИФ, Айсфьюри.  

* Наносит **82.5%** урона от силы заклинаний.
* Дает бафф на **15** секунд, увеличивающий урон **4** следующих [Ледяных шоков](https://ru.wowhead.com/spell=196840) на **300%**.
* Каждый усиленный [Ледяной шок](https://ru.wowhead.com/spell=196840) генерирует **8** ед. энергии Водоворота.
* Время восстановления **30** секунд.
* Наносит урон от магии <span style="color:#8fd5fe;font-size:1em;">**Льда**</span> и стоит **3%** маны.
* Генерирует **25** ед. энергии Водоворота и дополнительно **12** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* {{ site.data.spells.if }} имеет время полета снаряда около **1** секунды для максимальной дистанции, при этом бафф на урон [Ледяных шоков](https://ru.wowhead.com/spell=196840) накладывается сразу же по завершению каста.
* Урон [Ледяной ярости](https://ru.wowhead.com/spell=210714) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}, однако само заклинание наносит урона даже меньше чем {{ site.data.spells.lb }} и бафф по возможности лучше тратить на усиленный {{ site.data.spells.frs }}.
* Данный талант ощутимо усложняет ротацию в бою против одной цели, особенно для новичков. Альтернатива – {{ site.data.spells.primal_elem }}, дает чуть меньше урона, но гораздо проще в использовании.
* Если вы не можете идеально отыгрывать [Ледяную ярость](https://ru.wowhead.com/spell=210714) (без потерь баффов [Ледяного шока](https://ru.wowhead.com/spell=196840)), то лучше использовать [Повелителя изначальных стихий](https://ru.wowhead.com/spell=117013), это даст больше урона.
* После использования [Ледяной ярости](https://ru.wowhead.com/spell=210714) представьте, что вместо четырех следующих [Молний](https://ru.wowhead.com/spell=188196) вам нужно использовать четыре [Ледяных шока](https://ru.wowhead.com/spell=196840).
* В идеале каждый усиленный {{ site.data.spells.frs }} следует бафать талантом {{ site.data.spells.mote }}. 
* Не придерживайте стаки слишком долго, так как **потерять стак баффа [Ледяной ярости](https://ru.wowhead.com/spell=210714) гораздо хуже, чем потерять бафф от [Властелина стихий](https://ru.wowhead.com/spell=16166).**
* Если с талантом {{ site.data.spells.eote }} у вас есть два заряда [Выброса лавы](https://ru.wowhead.com/spell=51505), то используйте между ними усиленный [Ледяной шок](https://ru.wowhead.com/spell=196840).


<a href="https://ru.wowhead.com/spell=117014" target="blank" data-wh-icon-size="small" >**Удар духов стихий**</a> – талант второго тира, является частью ротации в основном билде против одной цели.

> Elemental Blast, EB, УДС.  

* Наносит **140%** урона от силы заклинаний.
* При завершении каста вы получаете бафф на **10** секунд на одну случайную вторичную характеристику – Критический удар, Скорость или Искусность.
* Время восстановления – **12** секунд.
* Время произнесения – **2** секунды.
* Наносит урон от <span style="color:#e600a6;font-size:1em;background: linear-gradient(94deg, #bd44b7 25%, #8064ea 50%, #fba520);-webkit-background-clip: text;-webkit-text-fill-color: transparent;"><strong>Силы стихий</strong></span> (сочетание школ магии <span style="color:#26d22b;font-size:1em;">**Природы**</span>, <span style="color:#f16f25;font-size:1em;">**Огня**</span> и <span style="color:#8fd5fe;font-size:1em;">**Льда**</span>) и стоит **2.75%** маны.
* Заклинание с уроном от <span style="color:#e600a6;font-size:1em;background: linear-gradient(94deg, #bd44b7 25%, #8064ea 50%, #fba520);-webkit-background-clip: text;-webkit-text-fill-color: transparent;"><strong>Силы стихий</strong></span> может применяться даже в том случае, если вам заблокировали любую школу магии, кроме всех трех одновременно.
* Однако, если вы получите прерывание во время каста [Удара духов стихий](https://ru.wowhead.com/spell=117014), то вы не сможете применять заклинания с уроном от сил  <span style="color:#26d22b;font-size:1em;">**Природы**</span>, <span style="color:#f16f25;font-size:1em;">**Огня**</span> или <span style="color:#8fd5fe;font-size:1em;">**Льда**</span>.
* Эффект легендарки {{ site.data.legend.ee }} срабатывает от [Удара духов стихий](https://ru.wowhead.com/spell=117014), без необходимости каста трех различных заклинаний.
* {{ site.data.spells.eb }} имеет время полета снаряда около **1** секунды для максимальной дистанции, при этом бафф на характеристику накладывается сразу же по завершению каста.
* Генерирует **30** ед. энергии Водоворота и дополнительно **15** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* Если вы скастуете  [Удар духов стихий](https://ru.wowhead.com/spell=117014) за **3** секунды до начала боя (а не за **2**, как рекомендуется на странице [ротации](https://stormkeeper.ru/ele/rotation.html)), то количество генерируемой энергии Водоворота может сократиться до **20** единиц, вместо **30**. Это связано с тем, что количество энергии на момент пула босса сокращается до **20%** от максимальной, чтобы не провоцировать игроков копить ресурс на треше ради дополнительного ДПС.
* Прок [Искусности](https://ru.wowhead.com/spell=168534) вылетает не сразу, а спустя примерно **1** секунду после завершения каста.
* Можно скастовать {{ site.data.spells.eb }} на **70** энергии Водоворота и сразу использовать {{ site.data.spells.es }}, тогда перегрузка вылетит уже после каста [Земного шока](https://ru.wowhead.com/spell=8042) и энергия Водоворота не уйдет в оверкап.
* После использования [Удара духов стихий](https://ru.wowhead.com/spell=117014) случайно дается один из трех баффов – **3%** к шансу Критического удара, **3%** к Скорости или **5.62%** Искусности.
* Прок [Искусности](https://ru.wowhead.com/spell=168534) гарантированно дает второй бафф, отличный от первого.
* Нет никакого смысла отслеживать на интерфейсе получаемые баффы от [Удара духов стихий](https://ru.wowhead.com/spell=117014), их следует воспринимать как пассивное улучшение, не влияющее на геймплей.
* В бою против **одной**, **двух** или **трёх** целей {{ site.data.spells.eb }} кастуется по кулдауну. В бою против **четырех** и более целей {{ site.data.spells.eb }} **не используется**. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Урон [Удара духов стихий](https://ru.wowhead.com/spell=117014) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}, но это не имеет значения – он все равно кастуется по КД (если не будет оверкапа энергии Водоворота) и если нету мгновенного прока [Выброса лавы](https://ru.wowhead.com/spell=51505) от [Волны лавы](https://ru.wowhead.com/spell=77756).


## АоЕ

<a href="https://ru.wowhead.com/spell=188443" target="blank" data-wh-icon-size="small" >**Цепная молния**</a> – основной генератор ресурса на АоЕ.

> Chain Lightning, CL, КЛ (одно из значений), Чайник (одно из значений), Цепнуха (одно из значений).  

* Наносит **63.5%** урона от силы заклинаний.
* Время произнесения – **2** секунды.
* Наносит урон от сил <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1%** маны.
* Генерирует по **4** ед. энергии Водоворота за каждую пораженную цель и дополнительно по **3** ед. при проке [Искусности](https://ru.wowhead.com/spell=168534).
* Эффективность [Искусности](https://ru.wowhead.com/spell=168534) для [Цепной молнии](https://ru.wowhead.com/spell=188443) **снижена до 33%**. То есть, если у вас **60%** Искусности, то [Цепная молния](https://ru.wowhead.com/spell=188443) имеет только **20%** на прок перегрузки.
* [Искусность](https://ru.wowhead.com/spell=168534) и шанс Критического удара просчитывается на каждую отдельную цель, вне зависимости от остальных.
* {{ site.data.spells.cl }} не имеет времени полета снаряда (потому что [очень быстро](https://ru.wikipedia.org/wiki/%D0%9C%D0%BE%D0%BB%D0%BD%D0%B8%D1%8F) двигается).
* Радиус перескока [Цепной молнии](https://ru.wowhead.com/spell=188443) составляет **5** метров и считается от края хитбокса цели, а не от центра.
* Изначально [Цепная молния](https://ru.wowhead.com/spell=188443) поражает максимум **3** цели, но на **43** уровне открывается пассивка [Цепная молния: Ранг 2](https://ru.wowhead.com/spell=231722), которая позволяет поражать в общей сложности **5** целей.
* [Цепная молния](https://ru.wowhead.com/spell=188443) используется на **двух** и более целях. С баффом от [Хранителя бурь](https://ru.wowhead.com/spell=191634) – начиная от **трех** целей. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html).
* Если во время АоЕ вам важен урон только в одну цель, то [Цепная молния](https://ru.wowhead.com/spell=188443) наносит столько же урона, что и [Молния](https://ru.wowhead.com/spell=188196) при поражении **3** целей и превосходит её при поражении **4+** целей (при учете, что урон от [Цепной молнии](https://ru.wowhead.com/spell=188443) получает только один моб, но энергию дает весь пак).
* Урон [Цепной молнии](https://ru.wowhead.com/spell=188443) **может** быть усилен на **20%** с помощью таланта четвертого тира {{ site.data.spells.mote }}.
* Под действием таланта [Перерождение](https://ru.wowhead.com/spell=114050) {{ site.data.spells.cl }} заменяется на альтернативную, «огненную» версию – [Поток лавы](https://ru.wowhead.com/spell=114074).
* [Поток лавы](https://ru.wowhead.com/spell=114074) **не получает** усиление урона на **20%** за счет таланта [Властелин стихий](https://ru.wowhead.com/spell=16166), однако поглощает его бафф.

<a href="https://ru.wowhead.com/spell=61882" target="blank" data-wh-icon-size="small" >**Землетрясение**</a> – спендер на АоЕ.

> Earthquake, EQ, ЕК, Квака, Землетряс, Лужа.

* Требует **60** ед. энергии Водоворота.
* Наносит **239%** урона от силы заклинаний за **6** секунд, то есть **39%** урона от силы заклинаний за **1** секунду.
* Наносит <span style="color:#FFFFFF;font-size:1em;">**Физический**</span> урон, но является заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span>.
* Игнорирует броню цели.
* Единственное заклинание Элема, которое усиливается дебаффом монаха на **5%** физического урона – [Таинственное прикосновение](https://ru.wowhead.com/spell=8647).
* Время действия [Землетрясения](https://ru.wowhead.com/spell=61882) и время между тиками **не меняется** от Скорости.
* Радиус эффекта составляет **8** метров.
* [Землетрясение](https://ru.wowhead.com/spell=61882) **стакается**, если положить несколько на одно и то же место.
* [Землетрясение](https://ru.wowhead.com/spell=61882) имеет **10%** шанс сбить врага с ног на **0.5** секунд.
* Данный эффект срабатывает максимум **один** раз за **одно** применение [Землетрясения](https://ru.wowhead.com/spell=61882) на **одного** врага. То есть нельзя сбить два раза одного моба одним [Землетрясением](https://ru.wowhead.com/spell=61882), но можно двумя.
* При использовании легендарки {{ site.data.legend.eogs }} урон [Землетрясения](https://ru.wowhead.com/spell=61882) увеличивается на **120%** после применения [Земного шока](https://ru.wowhead.com/spell=8042).
* [Землетрясение](https://ru.wowhead.com/spell=61882) имеет софт-кап на **20** целей. Подробнее в разделе [«Ограничение на количество целей»](https://stormkeeper.ru/info/target_cap.html).
* Чтобы [Землетрясение](https://ru.wowhead.com/spell=61882) нанесло больше урона, чем [Земной шок](https://ru.wowhead.com/spell=8042), оно должно сделать минимум **7** тиков. Это либо **1** секунда действия в бою против **7+** целей, либо **2** секунды в бою против **4** целей, либо **3** секунды в бою против **3** целей, либо **4** секунды в бою против **2** целей.
* Для использования [Землетрясения](https://ru.wowhead.com/spell=61882) без подтверждения выбора зоны, рекомендуется использовать следующий макрос:
~~~
#showtooltip
/cast [@cursor] Землетрясение
~~~
* Чтобы не терять в бою курсор, рекомендуется использовать [эту WA](https://wago.io/BS5jGO-Gr).

# Бурсты

<a href="https://ru.wowhead.com/spell=198067" target="blank" data-wh-icon-size="small" >**Элементаль огня**</a> – базовый [«бурст»](https://youtu.be/tz8rJDYF_yQ?t=5) с кулдауном **2.5** минуты.

>  Fire Elemental, FE, Fire Ele, Fred, ФЕ, Фаер элем, Фаер, Игнат.

* При активации способности призывается элементаль, который атакует **одного** противника раз в **3** секунды с помощью способности [Огненный взрыв](https://ru.wowhead.com/spell=57984/), которая наносит **67.5%** урона от силы заклинаний.
* [Элементаль огня](https://ru.wowhead.com/spell=198067) призывается мгновенно, считается заклинанием магии <span style="color:#f16f25;font-size:1em;">**Огня**</span> и стоит **5%** маны.
* Помимо обычной атаки, [Элементаль огня](https://ru.wowhead.com/spell=198067) также усиливает заклинание [Огненный шок](https://ru.wowhead.com/spell=188389):
  * Время между тиками сокращается на **25%**, вместо базовых **2** секунд оно составляет **1.5** секунды.
  * Продлевает действие новых [Огненных шоков](https://ru.wowhead.com/spell=188389) на **100%**, то есть до **36** секунд вместо **18** секунд.
* [Огненные шоки](https://ru.wowhead.com/spell=188389), наложенные до призыва [Элементаля огня](https://ru.wowhead.com/spell=198067), **не продлеваются**, но время между тиками у них **сокращается**.
* Базовая длительность [Элементаля огня](https://ru.wowhead.com/spell=198067) составляет **30** секунд, но проводник [Призыв пламени](https://ru.wowhead.com/spell=338303/%D0%BF%D1%80%D0%B8%D0%B7%D1%8B%D0%B2-%D0%BF%D0%BB%D0%B0%D0%BC%D0%B5%D0%BD%D0%B8?ilvl=226) продлевает его на **12.3** секунды, то есть до **42.3** секунд. Это время меняется от уровня проводника.
* [Элементаль огня](https://ru.wowhead.com/spell=198067) получает усиления от всех характеристик (Интеллект, Критический удар, Скорость, Универсальность), кроме Искусности.
* Сила [Элементаля огня](https://ru.wowhead.com/spell=198067) меняется динамически от ваших статов, то есть не имеет значения, когда использовать [Жажду крови](https://ru.wowhead.com/spell=2825) или прожимные аксессуары, до или после призыва.
* Имеет смысл придержать использование [Элементаля огня](https://ru.wowhead.com/spell=198067) под [Жажду крови](https://ru.wowhead.com/spell=2825), но только в том случае, если это не уменьшит количество его использований за бой. 
* При призыве [Элементаля огня](https://ru.wowhead.com/spell=198067) до пула он не начинает автоматически атаковать выбранную цель. Для этого следует сначала применить любое атакующее заклинание.
* По умолчанию, если цель находится дальше **40** метров от персонажа, то [Элементаль огня](https://ru.wowhead.com/spell=198067) не призовется. Это можно исправить с помощью следующего макроса:
```
#showtooltip
/cast [@player] Элементаль огня
```
* [Элементаль огня](https://ru.wowhead.com/spell=198067) – достаточно слабый бурст, особенно если сравнить с бурстами других классов (например, [Возгорание](https://ru.wowhead.com/spell=190319) Мага). 
* [Элементаль огня](https://ru.wowhead.com/spell=198067) лучше всего себя раскрывает в бою против **2-3** целей за счет нескольких усиленных [Огненных шоков](https://ru.wowhead.com/spell=188389).
* Легендарка {{ site.data.legend.fire }} повышает вероятность критического удара [Огненного шока](https://ru.wowhead.com/spell=188389) на **50%**, и каждый критический тик сокращает кулдаун [Элементаля огня](https://ru.wowhead.com/spell=198067) на **1** секунду.
* Усиленный талантом {{ site.data.spells.primal_elem }} {{ site.data.spells.fe }} получает следующие способности:
  * [Огненный взрыв](https://ru.wowhead.com/spell=57984/) – наносит больше урона, **121.5%** от силы заклинаний. Базовое время произнесения – **3** секунды. Кастуется элементалем автоматически.
  * [Жертвенный огонь](https://ru.wowhead.com/spell=118297) – ДоТ на одну цель с КД **10** секунд и временем произнесения **2** секунды. Наносит **36%** урона от силы заклинаний при наложении и **94.5%** урона от силы заклинаний в течение **21** секунд. Кастуется элементалем автоматически по КД. Кастуется автоматически, даже с отключенным автокастом.
  * {{ site.data.spells.meteor }} – активная способность, прожимается игроком с панели питомца или с помощью [макроса](https://stormkeeper.ru/info/macros.html#%D1%81%D1%82%D0%B8%D1%85%D0%B8%D0%B8). Наносит **135%** урона от силы заклинаний **8** противникам в радиусе **10** метров от основной цели.
     * Если использовать {{ site.data.spells.meteor }}, пока {{ site.data.spells.fe }} произносит [Огненный взрыв](https://ru.wowhead.com/spell=57984/) или [Жертвенный огонь](https://ru.wowhead.com/spell=118297), то их каст прервется и начнется заново.
    * Придерживайте использование этой способности под дополнительных аддов, если они появятся в течение жизни [Элементаля огня](https://ru.wowhead.com/spell=198067), либо используйте сразу с пула в бою против одной цели (пока активны различные баффы типа [Оссеиновый самоцвет](https://ru.wowhead.com/spell=326572) или {{ site.data.loot.Potion_of_Spectral_Intellect }}).
    * [Метеорит](https://ru.wowhead.com/spell=117588) имеет дальность применения **40** метров, но это расстояние считается не от края хитбокса цели (как для всех остальных заклинаний), а от центра.
    * Бывают ситуации, когда [Элементаль огня](https://ru.wowhead.com/spell=198067) может атаковать цель с помощью [Огненного взрыва](https://ru.wowhead.com/spell=57984/), но не достает [Метеоритом](https://ru.wowhead.com/spell=117588). 
    * {{ site.data.spells.meteor }} может применяться к союзников, что бывает полезно в те моменты, когда [Метеорит](https://ru.wowhead.com/spell=117588) невозможно использовать из-за большого хитбокса цели (например, как на Мве'зале в Той Стороне).
* С талантом {{ site.data.spells.primal_elem }} {{ site.data.spells.fe }} считается **«питомцем»**, а без него – **«стражем»**.
* С этим талантом невозможно призвать одновременно [Элементаля огня](https://ru.wowhead.com/spell=198067) и [Элементаля земли](https://ru.wowhead.com/spell=198103), так как последний призванный элементаль заменяет предыдущего. Не может быть двух питомцев одновременно, а стражей – сколько угодно.
* Также с этим талантом появляется возможность управлять [Элементалем огня](https://ru.wowhead.com/spell=198067), например с помощью команды «Отправить» вы можете указать, куда ему следует подойти.

<a href="https://ru.wowhead.com/spell=192249" target="blank" data-wh-icon-size="small" >**Элементаль бури**</a> – талант четвертого тира, заменяет [Элементаля огня](https://ru.wowhead.com/spell=198067). Ускоряет время произнесения [Молнии](https://ru.wowhead.com/spell=188196) и [Цепной молнии](https://ru.wowhead.com/spell=188443) на **3%** за каждый каст этих заклинаний.

> Storm Elemental, Storm ele, Storm, Шторм элем, Буря, Элем молний, Борис.

* При активации способности призывается элементаль, который атакует **одного** противника раз в **2** секунды с помощью способности [Порыв ветра](https://ru.wowhead.com/spell=157331/), нанося **15%** урона от силы заклинаний.
* [Элементаль бури](https://ru.wowhead.com/spell=192249) призывается мгновенно, считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Пока [Элементаль бури](https://ru.wowhead.com/spell=192249) жив, каждый каст [Молнии](https://ru.wowhead.com/spell=188196) и [Цепной молнии](https://ru.wowhead.com/spell=188443) дает один стак [Порыва ветра](https://ru.wowhead.com/spell=263806), который ускоряет время произнесения этих заклинаний на **3%**.
* Максимум **10** стаков, то есть **30%** снижение времени произнесения.
* Приоритет заклинаний меняется в зависимости от количества стаков [Порыва ветра](https://ru.wowhead.com/spell=157331/), подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html)
* [Элементаль бури](https://ru.wowhead.com/spell=192249) **не влияет** на [Огненный шок](https://ru.wowhead.com/spell=188389), так как, по сути, заменяет [Элементаля огня](https://ru.wowhead.com/spell=198067). Время между тиками не сокращается и длительность не увеличивается.
* Базовая длительность [Элементаля бури](https://ru.wowhead.com/spell=192249) составляет **30** секунд, но проводник [Призыв пламени](https://ru.wowhead.com/spell=338303/%D0%BF%D1%80%D0%B8%D0%B7%D1%8B%D0%B2-%D0%BF%D0%BB%D0%B0%D0%BC%D0%B5%D0%BD%D0%B8?ilvl=226) продлевает его на **12.3** секунды, то есть до **42.3** секунд. Это время меняется от уровня проводника.
* [Элементаль бури](https://ru.wowhead.com/spell=192249) получает усиления от всех характеристик (Интеллект, Критический удар, Скорость, Универсальность), кроме Искусности.
* Сила [Элементаля бури](https://ru.wowhead.com/spell=192249) меняется динамически от ваших статов, то есть не имеет значения, когда использовать [Жажду крови](https://ru.wowhead.com/spell=2825) или прожимные аксессуары, до или после призыва.
* По умолчанию, если цель находится дальше **40** метров от персонажа, то [Элементаль бури](https://ru.wowhead.com/spell=192249) не призовется. Это можно исправить с помощью следующего макроса (он рабочий, доверьтесь мне):
```
#showtooltip
/cast [@player] Элементаль огня
```
* [Элементаль бури](https://ru.wowhead.com/spell=192249) является очень сильным бурстом на АоЕ, но не в бою против одной цели.
* Легендарка {{ site.data.legend.fire }} в том числе работает и на [Элементаля бури](https://ru.wowhead.com/spell=192249), сокращая его время восстановления на **1** секунду за каждый каждый критический тик [Огненного шока](https://ru.wowhead.com/spell=188389). 
* [Элементаль бури](https://ru.wowhead.com/spell=192249) практически никогда не используется без взятого таланта {{ site.data.spells.primal_elem }}.
* Усиленный талантом {{ site.data.spells.primal_elem }}, {{ site.data.spells.storm }} получает следующие способности:
  * [Порыв ветра](https://ru.wowhead.com/spell=157331) – наносит больше урона, **27%** урона от силы заклинаний. Базовое время произнесения – **2** секунды. Применяется элементалем автоматически.
  * [Призыв молнии](https://ru.wowhead.com/spell=157348) – бафф всего урона [Элементаля бури](https://ru.wowhead.com/spell=192249) на **300%**, кастуется автоматически, сразу после призыва. Также наносит **54%** урона от силы заклинаний. Время восстановления **10** секунд. Кастуется автоматически, даже с отключенным автокастом.
  * [Око шторма](https://ru.wowhead.com/spell=157375/) – активная способность, прожимается игроком с панели питомца или с помощью [макроса](https://stormkeeper.ru/info/macros.html#%D1%81%D1%82%D0%B8%D1%85%D0%B8%D0%B8). Раз в секунду наносит **18%** урона от силы заклинаний цели и противникам вокруг неё, максимум **6** целей. Урон усиливается на **5%** каждый раз, когда эта способность наносит урон. Перед использованием убедитесь, что [Элементаля бури](https://ru.wowhead.com/spell=192249) использовал на себя [Призыв молнии](https://ru.wowhead.com/spell=157348) и имеет бафф на **300%** наносимого урона! [WA для отслеживания](https://wago.io/oiozoI5RD).
* С талантом {{ site.data.spells.primal_elem }} {{ site.data.spells.storm }} считается **«питомцем»**, а без него – **«стражем»**.
* С этим талантом невозможно призвать одновременно [Элементаля бури](https://ru.wowhead.com/spell=192249) и [Элементаля земли](https://ru.wowhead.com/spell=198103), так как последний призванный элементаль заменяет предыдущего. Не может быть двух питомцев одновременно, а стражей – сколько угодно.
* Также с этим талантом появляется возможность управлять [Элементалем бури](https://ru.wowhead.com/spell=192249), например с помощью команды «Отправить» вы можете указать, куда ему следует подойти.
* В сравнении с другими элементалями, [Элементаль бури](https://ru.wowhead.com/spell=192249) имеет гораздо меньшую скорость передвижения. Но это заметно только в том случае, когда он передвигается отдельно от персонажа.
* При призыве [Элементаля бури](https://ru.wowhead.com/spell=192249), он сразу же начинает кастовать [Призыв молнии](https://ru.wowhead.com/spell=157348) в текущую цель.

<a href="https://ru.wowhead.com/spell=191634" target="blank" data-wh-icon-size="small" >**Хранитель бурь**</a> – талант седьмого тира, бурст с кулдауном **1** минута, усиливает две следующих [Молнии](https://ru.wowhead.com/spell=188196) или [Цепные молнии](https://ru.wowhead.com/spell=188443).

> Stormkeeper, SK, Keeper, СК, ХБ (не стоит), Штормкипер, Кипер.

* Дает отличный бурст как по одной цели, так и на АоЕ.
* Не зависит от передвижения, как другие таланты этого тира.
* Каждую усиленную [Молнию](https://ru.wowhead.com/spell=188196) и [Цепную молнию](https://ru.wowhead.com/spell=188443) желательно бафать [Властелином стихий](https://ru.wowhead.com/spell=16166).
* На **1-2** цели следует использовать [Молнию](https://ru.wowhead.com/spell=188196), а на **3+** уже [Цепную молнию](https://ru.wowhead.com/spell=188443). Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html). 
* Вы можете придерживать кулдаун [Хранителя бурь](https://ru.wowhead.com/spell=191634) под важные моменты боя, например, сломанная колонна на Грязешмяке или на копии Баргаста в бою против Ловчего Альтимора.
* Количество получаемой энергии Водоворота и наносимый урон в зависимости от количества пораженных целей и считается по формуле ([количество целей] * 4 + [количество целей] * 3):
1. {{ site.data.spells.lb }} по **1** цели дает **11** энергии Водоворота и наносит **439.3%** урона от силы заклинаний.
2. {{ site.data.spells.cl }} по **2** целям дает **20** энергии Водоворота и наносит **253.8%** урона от силы заклинаний.
3. {{ site.data.spells.cl }} по **3** целям дает **39** энергии Водоворота и наносит **500.55%** урона от силы заклинаний.
4. {{ site.data.spells.cl }} по **4** целям дает **64** энергии Водоворота и наносит **827.2%** урона от силы заклинаний.
5. {{ site.data.spells.cl }} по **5** целям дает **95** энергии Водоворота и наносит **1233.75%** урона от силы заклинаний. 
6. {{ site.data.spells.cl }} по **6** целям дает **132** энергии Водоворота.
* [Хранитель бурь](https://ru.wowhead.com/spell=191634) **не увеличивает** урон [Цепной молнии](https://ru.wowhead.com/spell=188443), в отличие от обычной [Молнии](https://ru.wowhead.com/spell=188196), а только гарантирует прок [Искусности](https://ru.wowhead.com/spell=168534) по всем целям.


<a href="https://ru.wowhead.com/spell=114050" target="blank" data-wh-icon-size="small" >**Перерождение**</a> – талант седьмого тира, бурст с кулдауном **3** минуты, позволяет в течение **15** секунд использовать {{ site.data.spells.lvb }} без кулдауна и он наносит дополнительный урон, равный вашей вероятности Критического удара

* Проигрывает [Хранителю бурь](https://ru.wowhead.com/spell=191634) практически по всем параметрам – урон, кулдаун, удобство использования, влияние передвижения и т.д.
* При активации обновляет [Огненный шок](https://ru.wowhead.com/spell=188389) на всех целях и использует на них [Выброс лавы](https://ru.wowhead.com/spell=51505).
* Список приоритетов во время активного [Перерождения](https://ru.wowhead.com/spell=114049) можно посмотреть в [соответствующем разделе](https://stormkeeper.ru/ele/rotation.html).
* {{ site.data.spells.cl }} под действием [Перерождения](https://ru.wowhead.com/spell=114049) превращается в [Поток лавы](https://ru.wowhead.com/spell=114074), увеличивая свой урон на **10%** за каждую пораженную дополнительную цель.
* В случае перебежки, чтобы не терять урон от [Перерождения](https://ru.wowhead.com/spell=114049), необходимо использовать {{ site.data.spells.swg }}, а также придерживать его кулдаун в течение минуты под следующее {{ site.data.spells.asc_ele}}.

# Пассивные способности



<a href="https://ru.wowhead.com/spell=60188" target="blank" data-wh-icon-size="small" >**Неистовство стихий**</a> – увеличивает критический урон заклинаний на **250%** вместо **200%**.

* Позволяет Элем Шаману наносить больше Критического урона всеми своими способностями, включая способности Ковенантов.
* Работает как «белый список», то есть действует только на те способности и заклинания, которые указаны на странице Wowhead.
* **Не действует** на аксессуары.
* **Не действует** на предметы и баффы в Смертельной тризне: [Кровавое копье](https://ru.wowhead.com/spell=328351), [Забытый кузнечный молот](https://ru.wowhead.com/spell=328128), [Освобожденная анима](https://ru.wowhead.com/spell=328404).
* **Не действует** на способности [Элементаля огня](https://ru.wowhead.com/spell=198067) и [Элементаля бури](https://ru.wowhead.com/spell=192249).
* Работает на исцеляющие заклинания, за счет пассивной способности на **52** уровне: [Неистовство стихий: Ранг 2](https://ru.wowhead.com/spell=343190).
* В PvP Критический удар для Элем Шамана увеличивает урон на **225%**, тогда как у остальных классов – на **175%**.

<a href="https://ru.wowhead.com/spell=77756" target="blank" data-wh-icon-size="small" >**Волна лавы**</a> – периодический урон от [Огненного шока](https://ru.wowhead.com/spell=188389) с вероятностью **10%** может восстановить [Выброс лавы](https://ru.wowhead.com/spell=51505) и сделать его мгновенным.

* Наш основной прок, который оказывает большое влияние на ротацию как в бою против одной цели, так и на кливе.
* В бою против одной цели [Выброс лавы](https://ru.wowhead.com/spell=51505) под этим проком имеет очень высокий приоритет. Подробнее в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html). 
* Чем больше вы поддерживаете [Огненных шоков](https://ru.wowhead.com/spell=188389), тем больше будет проков.
* Чем больше у вас Скорости, тем чаще тикает [Огненный шок](https://ru.wowhead.com/spell=188389), что тоже увеличивает количество проков [Волны лавы](https://ru.wowhead.com/spell=77756). Аналогично при активном [Элементале огня](https://ru.wowhead.com/spell=198067).
* Талант первого тира [Эхо стихий](https://ru.wowhead.com/spell=333919/) дает [Выбросу лавы](https://ru.wowhead.com/spell=51505) второй заряд, в результате чего время восстановления не теряется при проке [Волны лавы](https://ru.wowhead.com/spell=77756), а также сокращает кулдаун [Выброса лавы](https://ru.wowhead.com/spell=51505) до **8** секунд вместо «**8** секунд + время произнесения + время реакции».




<a href="https://ru.wowhead.com/spell=16166" target="blank" data-wh-icon-size="small" >**Властелин стихий**</a> – пассивный талант, увеличивает урон от различных заклинаний на **20%** после применения [Выброса лавы](https://ru.wowhead.com/spell=51505).

> Master of the Elements, Mote, Моте.

* Работает на следующие способности:
  * **Сила природы** – {{ site.data.spells.es }}, {{ site.data.spells.lb }}, {{ site.data.spells.cl }}, [Гром и молния](https://ru.wowhead.com/spell=51490), [Волшебное переливание](https://ru.wowhead.com/spell=328923).
  * **Магия льда** – {{ site.data.spells.frs }}, {{ site.data.spells.if }}.
  * **Физический урон** – {{ site.data.spells.quake }}.
* Не работает на магию огня, то есть не усиливает {{ site.data.spells.lvb }} и {{ site.data.spells.fs }}.
* Бафф появляется сразу же после завершения каста [Выброса лавы](https://ru.wowhead.com/spell=51505), а не в момент попадания снаряда в цель.
* **Важный момент!** Если у вас есть два заряда [Выброса лавы](https://ru.wowhead.com/spell=51505), то не следует кастовать между ними обычную [Молнию](https://ru.wowhead.com/spell=188196), так как это приведет к потере урона. Это связано с тем, что если в момент применения [Молнии](https://ru.wowhead.com/spell=188196) у вас прокнет {{ site.data.spells.lava_surge }}, то вы потеряете часть КД на {{ site.data.spells.lvb }}.
* Но это правило не распространяется на мгновенные заклинания – усиленная {{ site.data.spells.lb }} от таланта {{ site.data.spells.sk }}, усиленный {{ site.data.spells.frs }} от таланта {{ site.data.spells.if }}, а также {{ site.data.spells.es }} и {{ site.data.spells.quake }}. Поглощайте бафф от [Властелина стихий](https://ru.wowhead.com/spell=16166) этими заклинаниями, даже если у вас уже есть один стак [Выброса лавы](https://ru.wowhead.com/spell=51505).
* Проще говоря – не стоит кастовать обычную [Молнию](https://ru.wowhead.com/spell=188196), если у вас есть заряд [Выброса лавы](https://ru.wowhead.com/spell=51505). Бафф от таланта {{ site.data.spells.mote }} не перекроет потенциальную потерю урона при проке {{ site.data.spells.lava_surge }}. 
* Подходя к **60** ед. энергии Водоворота, ориентируйтесь на кулдауны [Выброса лавы](https://ru.wowhead.com/spell=51505) и других заклинаний. Придерживайте {{ site.data.spells.es }}, чтобы скастовать его с баффом от {{ site.data.spells.mote }}.
* По сути, у нас есть окно между **60** и **100** энергии Водоворота, за время которого мы должны использовать [Выброс лавы](https://ru.wowhead.com/spell=51505) и сразу же {{ site.data.spells.es }}.
* Не стоит уходить в оверкап энергии Водоворота ради баффа [Властелина стихий](https://ru.wowhead.com/spell=16166), это ведет к потере урона.
* Приоритет заклинаний для баффа {{ site.data.spells.mote }}:

1. {{ site.data.covenants.ft }} в бою против **одной** цели.
2. {{ site.data.spells.quake }} на **две** и более цели, либо на **одну** цель при наличии баффа от [Отголосков Великого Раскола](https://ru.wowhead.com/spell=336215).
3. {{ site.data.spells.lb }} с баффом от [Хранителя бурь](https://ru.wowhead.com/spell=191634).
4. {{ site.data.spells.es }}.
5. {{ site.data.spells.frs }} с баффом от [Ледяной ярости](https://ru.wowhead.com/spell=210714).
6. {{ site.data.spells.eb }}.
  * **Важно!** {{ site.data.spells.eb }} используется всегда по кулдауну, вне зависимости от наличия или отсутствия баффа {{ site.data.spells.mote }}.
7. {{ site.data.spells.lb }}.
8. {{ site.data.spells.if }}.
9. {{ site.data.spells.frs }}.

# Лечение и защита

<a href="https://ru.wowhead.com/spell=108271" target="blank" data-wh-icon-size="small" >**Астральный сдвиг**</a> – основное защитное заклинание.

> Astral Shift, AS, Shift, АС, Шифт, Астрал, Сейв, Личка, Минус сорок.

* При использовании дается бафф, снижающий получаемый урон на **40%**.
* Длительность баффа – **12** секунд (только после **54** уровня и открытия пассивной способности [Астральный сдвиг: Ранг 2](https://ru.wowhead.com/spell=343196), до этого **8** секунд).
* Время восстановления – **1.5** минуты.
* Не имеет ГКД.
* **Не может** применяться во время оглушения, заморозки, паралича, страха и сна.
* Эффекты снижения получаемого урона складываются **мультипликативно**, то есть значения снижения урона перемножаются, а не складываются.
* [Астральный сдвиг](https://ru.wowhead.com/spell=108271) в сочетании с [Каменными доспехами](https://ru.wowhead.com/spell=118337), даст не **80%** сниженного урона, а **64%**.

Проценты снижения получаемого урона высчитываются не все разом (например, **40%** + **40%**), а как бы по порядку. То есть если вам наносится **1000** урона, то при активном [Астральном сдвиге](https://ru.wowhead.com/spell=108271) и [Каменных доспехах](https://ru.wowhead.com/spell=118337) сначала посчитается первый сейв, **1000**-**40%**, что равняется **600** урона. Потом от **600** урона отнимается еще раз **40%**, то есть **600**-**40%**=**360**. Таким же образом считается снижение урона от Универсальности и внешние сейвы, например [Железная кора](https://ru.wowhead.com/spell=102342) Друида и другие подобные заклинания.
{: class="bordered"}

<a href="https://ru.wowhead.com/spell=8004" target="blank" data-wh-icon-size="small" >**Исцеляющий всплеск**</a> – быстрый отхил одной цели.

> Healing Surge, HS, ХС, Всплеск, Флешка, Сурдж, Хилка.

* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **25%** маны.
* Исцеляет одну цель на **280%** от силы заклинаний.
* В подсказке на Wowhead указано **248%** от силы заклинаний, но у Элема отхил этой способности увеличен на **13%** за счет пассивной ауры – [Шаман - Стихии](https://ru.wowhead.com/spell=137040).
* Кастуется **1.5** секунды.
* В отличие от атакующих заклинаний, [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004) можно применять к союзнику, который стоит за вашей спиной.
* На [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004) оказывают влияние все характеристики, кроме Искусности.
* Критический эффект повышен на **250%** за счет пассивной способности на **52** уровне: [Неистовство стихий: Ранг 2](https://ru.wowhead.com/spell=343190). 
* Проводник [Освежающие воды](https://ru.wowhead.com/spell=337974) дополнительно увеличивает отхил по вам на **22%**.
* Крайне полезное заклинание для экстренного отхила как себя, так и союзника. Снимает один стак [Мучительного](https://ru.wowhead.com/affix=12) аффикса, а также полезно на [Взрывном](https://ru.wowhead.com/affix=11) аффиксе для отхила после смерти пака.

<a href="https://ru.wowhead.com/spell=5394" target="blank" data-wh-icon-size="small" >**Тотем исцеляющего потока**</a> – тотем, который дает небольшой пассивный отхил.

> Healing Stream Totem, HST, ХСТ, Тотемчик, Хил-тотем, Малыш.

* При использовании призывается тотем под ноги персонажа, который действует **15** секунд и раз в **2** секунды лечит одного союзника.
* Время восстановления – **30** секунд. 
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **9%** маны.
* За один тик хилит на **47%** от силы заклинаний, то есть суммарно **352.5%** от силы заклинаний за все время действия тотема.
* Скорость добавляет дополнительный тик в конце действия эффекта, ослабленный в зависимости от недостающего времени до «полного» тика.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* На каждом тике выбирает случайную раненую цель в радиусе действия тотема, **40** метров.
* Использование в бою [Тотем исцеляющего потока](https://ru.wowhead.com/spell=5394) ведет к снижению урона, так как вы тратите ГКД на его установку.
* Используйте [Тотем исцеляющего потока](https://ru.wowhead.com/spell=5394) в те моменты, когда невозможно атаковать, либо перед или после боя.

<a href="https://ru.wowhead.com/spell=1064" target="blank" data-wh-icon-size="small" >**Цепное исцеление**</a> – АоЕ отхил трёх союзников с долгим кастом.

* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **30%** маны.
* Исцеляет три цели, первую на **210%** от силы заклинаний, вторую на **147%**, третью на **103%**.
* Суммарное количество отхила – **460%** от силы заклинаний.
* Кастуется **2.5** секунды.
* Является «улучшенным смарт-хилом», то есть хилит в первую очередь цели с наименьшим количеством здоровья.
* Радиус перескока отхила составлявет **15** метров.
* В отличие от атакующих заклинаний, [Цепное исцеление](https://ru.wowhead.com/spell=1064) можно применять к союзнику, который стоит за вашей спиной.
* На [Цепное исцеление](https://ru.wowhead.com/spell=1064) оказывают влияние все характеристики, кроме Искусности.
* Критический эффект повышен на **250%** за счет пассивной способности на **52** уровне: [Неистовство стихий: Ранг 2](https://ru.wowhead.com/spell=343190). 
* Снимает по стаку [Мучительного](https://ru.wowhead.com/affix=12) аффикса с каждого отхиленного союзника.
* [Цепное исцеление](https://ru.wowhead.com/spell=1064) используется гораздо реже, чем [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004), так как дольше кастуется и требует нахождения союзников в радиусе **15** метров друг от друга.
* Элем шаман может вообще не выносить это заклинание на панель и использовать для отхила только [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004).
* Тем не менее, [Цепное исцеление](https://ru.wowhead.com/spell=1064) может быть полезно в те моменты, когда группа равномерно получает урон и вы не имеете возможность атаковать, например, переходка на последнем боссе в Шпилях Перерождения.

<a href="https://ru.wowhead.com/spell=974" target="blank" data-wh-icon-size="small" >**Щит земли**</a> – талант третьего тира, хилит при получении урона.

> Earth Shield, ES, Земляной щит, Ерсшилд.

* Вешает бафф на **10** минут, который можно применить к игроку или союзнику.
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Невозможно наложить одновременно [Щит молний](https://ru.wowhead.com/spell=192106) и [Щит земли](https://ru.wowhead.com/spell=974), так как активен может быть только один щит стихий одновременно.
* [Щит земли](https://ru.wowhead.com/spell=974) усиливает все исцеление игрока, наложившего бафф, на **20%**.
* Бафф на увеличение исцеления действует только для того шамана, кто наложил {{ site.data.spells.eshield }}.
* Исцеление аксессуаров **не усиляется** на **20%**.
* Увеличение отхила работает как «белый список», только на способности шамана. Полный список есть на странице Wowhead.
* Помимо этого, каждый раз, когда цель с [Щитом земли](https://ru.wowhead.com/spell=974) получает урон, она хилится на **73.5%** от силы заклинаний. 
* Имеет **9** зарядов на срабатывание отхила, после чего [Щит земли](https://ru.wowhead.com/spell=974) спадает и его надо обновлять.
* Внутренний кулдаун на лечение при получении урона – **3** секунды.
* {{ site.data.spells.eshield }} от другого шамана никак не взаимодействует с вашим. Они могут срабатывать как вместе, так и по отдельности.
* Легендарный предмет [Земная гармония](https://ru.wowhead.com/spell=335886) увеличивает эффективность отхила [Щита земли](https://ru.wowhead.com/spell=974) на **150%**, если у цели на момент получения урона и проке эффекта было менее **75%** здоровья.
* Проводник [Объятия земли](https://ru.wowhead.com/spell=338329) дополнительно увеличивает получаемое исцеление от ваших заклинаний по цели с [Щитом земли](https://ru.wowhead.com/spell=974) на **8%**.

<a href="https://ru.wowhead.com/spell=108281" target="blank" data-wh-icon-size="small" >**Наставления предков**</a> – талант пятого тира, дает отхил при нанесении урона.

> Ancestral Guidance, AG, Наставления, АГ.

* При использовании дается бафф на **10** секунд, при котором вы лечите трех союзников на **25%** от нанесенного урона или исцеления.
* Время восстановления – **2** минуты.
* Считается заклинанием школы магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Применяется мгновенно и не имеет ГКД.
* Имеет хард-кап в **3** цели. 
* Радиус лечения – **40** метров.
* Если цель будет одна, то отхил не увеличится, если больше трёх – лечение получат только три.
* Является «смарт-хилом», то есть хилит только раненые цели.
* Лучше всего [Наставления предков](https://ru.wowhead.com/spell=108281) хилит во время АоЕ, так как там вы наносите больше урона.
* Для максимальной эффективности отхила во время [Наставления предков](https://ru.wowhead.com/spell=108281) можно использовать [Цепное исцеление](https://ru.wowhead.com/spell=1064) или [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004), так как лечение с этих заклинаний тоже будет дублироваться талантом.




<a href="https://ru.wowhead.com/spell=198103" target="blank" data-wh-icon-size="small" >**Элементаль земли**</a> – призыв элементаля со слабой атакой, который забирает на себя окружающих мобов. 

> Earth Elemental, Earth, Steve, Элем земли, Игорь, Стив. 

* При активации способности призывается Элементаль, который атакует **одного** противника в ближнем бою, нанося **25%** урона от силы заклинаний раз в **2** секунды. Тип урона – <span style="color:#FFFFFF;font-size:1em;">**Физический**</span>.
* Длительность [Элементаля земли](https://ru.wowhead.com/spell=198103) составляет **60** секунд.
* Время восстановления – **5** минут.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) призывается мгновенно, считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Имеет агро-ауру вокруг себя, благодаря которой может переагрить мобов даже с танка.
* Помимо этого, Элементаль также имеет АоЕ таунт, который срабатывает при следующих условиях:
  1. Любой **не-танк** получает агро (включая деревья Совы).
  2. Танк отошел на **20** метров от заагренной цели.
  3. Здоровье танка опустилось ниже **35%**.
* **Не таунтит боссов**, но может забрать на себя его прислужников. На боях, где необходимо точное позиционирование мобов, его лучше не призывать.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) получает усиления от всех характеристик (Интеллект, Критический удар, Скорость, Универсальность), кроме Искусности.
* [Элементаль земли](https://ru.wowhead.com/spell=198103) имеет **150%** здоровья и **600%** брони шамана, а шанс критического удара мобов по нему снижен на **6%**.
* Сила [Элементаля земли](https://ru.wowhead.com/spell=198103) меняется динамически от ваших статов, то есть не имеет значения, когда использовать [Жажду крови](https://ru.wowhead.com/spell=2825) или прожимные аксессуары, до или после призыва.
* Однако его здоровье снапшотится в момент призыва. Чем больше у вас максимального ХП – тем толще [Элементаль земли](https://ru.wowhead.com/spell=198103).
* Проводник [Прирост жизненных сил](https://ru.wowhead.com/spell=337981) увеличивает ваше здоровье на **29%**, что превращает [Элементаля земли](https://ru.wowhead.com/spell=198103) в защитный кулдаун.
* Легендарка {{ site.data.legend.stone }} дает [Элементалю земли](https://ru.wowhead.com/spell=198103) производить вокруг себя [Землетрясение](https://ru.wowhead.com/spell=61882) все время своей жизни.
  * Сбитие с ног от [Землетрясения](https://ru.wowhead.com/spell=61882) срабатывает всего один раз на одного моба.
  * Под действием этой легендарки [Элементаль земли](https://ru.wowhead.com/spell=198103) может переагрить на себя мобов и умереть.
  * С этой легендаркой при призыве [Элементаля земли](https://ru.wowhead.com/spell=198103) поглощается эффект таланта {{ site.data.spells.mote }} и [Землетрясение](https://ru.wowhead.com/spell=61882) усиливается на **20%** на все время своего действия.
* При призыве [Элементаля земли](https://ru.wowhead.com/spell=198103), он сразу же мчится к вашей цели. Чтобы не запулить босса раньше времени, используйте следующий макрос:
```
#showtooltip
/cast [@player] Элементаль земли
```
* Усиленный талантом {{ site.data.spells.primal_elem }}, {{ site.data.spells.igor }} увеличивает свою атаку на **80%** и получает следующие способности:
  * [Разгневанный дух земли](https://ru.wowhead.com/spell=36213) – активная способность, генерирует большое количество угрозы раз в **5** секунд в радиусе **15** метров. Применяется элементалем автоматически.
  * [Раздавить](https://ru.wowhead.com/spell=118345/) – оглушение одной цели на **4** секунды. Применяется автоматически сразу после вызова [Элементаля земли](https://ru.wowhead.com/spell=198103). Имеет время восстановления **40** секунд, то есть применяется два раза за один призыв элема.
  * [Каменные доспехи](https://ru.wowhead.com/spell=118337/) – активная способность, прожимается игроком с панели питомца или с помощью [макроса](https://stormkeeper.ru/info/macros.html#%D1%81%D1%82%D0%B8%D1%85%D0%B8%D0%B8). Уменьшает получаемый урон [Элементалем земли](https://ru.wowhead.com/spell=198103) и **игроком** на **40%** на **10** секунд. Элементаль применяет эту способность автоматически, когда у него остается мало здоровья. В сочетании с [Астральным сдвигом](https://ru.wowhead.com/spell=108271/) снижает получаемый игроком урон на **64%**.  
* С талантом {{ site.data.spells.primal_elem }} {{ site.data.spells.igor }} считается **«питомцем»**, а без него – **«стражем»**.
* С этим талантом невозможно призвать одновременно [Элементаля земли](https://ru.wowhead.com/spell=198103) и [Элементаля огня](https://ru.wowhead.com/spell=198067), так как последний призванный элементаль заменяет предыдущего. Не может быть двух питомцев одновременно, а стражей – сколько угодно.
* Также с этим талантом появляется возможность управлять [Элементалем земли](https://ru.wowhead.com/spell=198103), например с помощью команды «Отправить» вы можете указать, куда ему следует подойти.
* Аналогом [Элементаля земли](https://ru.wowhead.com/spell=198103) являются древни Друида – [Сила природы](https://ru.wowhead.com/spell=205636), но они чаще вызываются и лучше держат на себе мобов.

<a href="https://ru.wowhead.com/spell=79206" target="blank" data-wh-icon-size="small" >**Благосклонность предков**</a> – позволяет применять заклинания на ходу.

> Spiritwalker's Grace, SWG, СВГ, Спиритволк, Каст на ходу.  

* Длительность баффа – **15** секунд.
* Время восстановления – **2** минуты.
* Не имеет ГКД.
* Может применяться во время произнесения другого заклинания.
* [Благосклонность предков](https://ru.wowhead.com/spell=79206) считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **14%** маны.
* Работает как «белый список», то есть действует только на те способности и заклинания, которые указаны на странице Wowhead.
* [Благосклонность предков](https://ru.wowhead.com/spell=79206) позволяет не терять урон на перебежках, но кулдаун в **2** минуты не позволит слишком часто её использовать.
* В Рейде [Благосклонность предков](https://ru.wowhead.com/spell=79206) стоит использовать под определенные механики, например, вынос какого-нибудь дебаффа или запланированная перебежка.
* В Мифик+ обычно используется на боссах аналогично ситуациям в рейде или на подходе к паку для занятия удобной позиции.  
* Подробности про урон в движении есть в разделе [«Ротация и приоритеты»](https://stormkeeper.ru/ele/rotation.html#%D1%83%D1%80%D0%BE%D0%BD-%D0%B2-%D0%B4%D0%B2%D0%B8%D0%B6%D0%B5%D0%BD%D0%B8%D0%B8)



<a href="https://ru.wowhead.com/spell=370" target="blank" data-wh-icon-size="small" >**Развеивание магии**</a> – снимает с противника один благотворный магический эффект.

> Purge, Пурдж, Диспел (одно из значений), Очищение, Снятие магии.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Тратит ГКД, но не имеет времени восстановления.
* Магические эффекты снимаются только с противников, но не с союзников.
* Магический бафф отображается на противнике как иконка с белой рамкой.
* Настроить отображение можно с помощью [WA](https://wago.io/r2QBcOnlT) или [мода на Plater](https://wago.io/turHC0a5k).
* Радиус действия [Развеивания магии](https://ru.wowhead.com/spell=370) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* Крайне полезное заклинание в PvP и в Мифик+.
* Аналоги [Развеивания магии](https://ru.wowhead.com/spell=370) у других классов:
  * [Рассеивание заклинаний](https://ru.wowhead.com/spell=528/) **Жреца** (без КД).
  * [Усмиряющий выстрел](https://ru.wowhead.com/spell=19801) **Охотника** (**10** секунд КД).
  * [Поглощение магии](https://ru.wowhead.com/spell=278326) **Охотника на демонов** (**10** секунд КД).
  * [Чарокрад](https://ru.wowhead.com/spell=30449) **Мага** (ворует заклинание, а не снимает, без КД).
  * [Волшебный поток](https://ru.wowhead.com/spell=28730) **Кровавых эльфов** (**2** минуты КД).
  * [Пожирание магии](https://ru.wowhead.com/spell=19505) питомца **Чернокнижника** (**15** секунд КД).
* В Мифик+ желательно снимать следующие магические баффы:
  * **Та Сторона**:
    * [Объятия смерти](https://ru.wowhead.com/spell=333875) **Вестника смерти**, если не сбили каст (лучше оставить под [Чарокрад](https://ru.wowhead.com/spell=30449) Магов).
    * [Призрачный](https://ru.wowhead.com/spell=344739) **Духа вестника смерти** из племени Атал'ай.
  * **Чертоги Покаяния**:
    * [Обращение в камень](https://ru.wowhead.com/spell=326607) **Камнерожденного разорителя**, если не сбили каст.
  * **Туманы Тирна Скитта**:
    * [Последний вздох](https://ru.wowhead.com/spell=322968) **Друста-злобный коготь**, либо после смерти моба снять проклятие с союзника с помощью [Очищения духа](https://ru.wowhead.com/spell=51886).
    * [Плащ с колючками](https://ru.wowhead.com/spell=324776) **Туманного культиватора**, если не сбили каст.
    * [Покровительство леса](https://ru.wowhead.com/spell=324914) **Туманной хранительницы**, если не сбили каст.
    * [Стимуляция сопротивляемости](https://ru.wowhead.com/spell=326046) **Иглобрюха-рогача**, если не сбили каст.
  * **Чумные Каскады**:
    * [Чудесный рост](https://ru.wowhead.com/spell=328016/) **Грибомага**, если не сбили каст (лучше оставить под [Чарокрад](https://ru.wowhead.com/spell=30449) Магов).
    * [В слизи](https://ru.wowhead.com/spell=328662/) **различных слизней**, если лужа после смерти моба будет мешать.
  * **Кровавые Катакомбы**:
    * [Каменная кожа](https://ru.wowhead.com/spell=322433) **Часового чертогов**, если не сбили каст.
  * **Шпили Перерождения**:
    * [Усиление оружия](https://ru.wowhead.com/spell=327332) **Раскольника-целителя**.
    * [Усиление оружия](https://ru.wowhead.com/spell=327655) **Раскольника-защитника**.
    * [Благословение оружия](https://ru.wowhead.com/spell=328288) **Раскольника-хранителя**.
  * **Смертельная Тризна**:
    * [Теневая завеса](https://ru.wowhead.com/spell=335141) **Нар'зуды**.
  * **Театр Боли**:
    * [Нечестивое рвение](https://ru.wowhead.com/spell=341902) **Боевого ритуалиста**, если не сбили каст.
    * [Призрачное перенаправление](https://ru.wowhead.com/spell=320272) на **первом боссе**.
    * [Костяной щит](https://ru.wowhead.com/spell=333293) **Костяного чародея** (лучше оставить под [Чарокрад](https://ru.wowhead.com/spell=30449) Магов).


<a href="https://ru.wowhead.com/spell=51886" target="blank" data-wh-icon-size="small" >**Очищение духа**</a> – снятие проклятия с союзника.

> Cleanse Spirit, Decurse, Очищение, Декурс, Диспел.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **6.5%** маны.
* Время восстановления составляет **8** секунд, но только в том случае, если было снято проклятие, иначе потратится только ГКД и мана.
* [Возрождение духов](https://ru.wowhead.com/spell=77130), аналог [Очищения духов](https://ru.wowhead.com/spell=51886) у Рестор шамана, снимает не только проклятия, но и магию с союзников.
* Аналоги [Очищения духов](https://ru.wowhead.com/spell=51886) у других классов:
  * [Снятие порчи](https://ru.wowhead.com/spell=2782) **Друида**.
  * [Снятие проклятия](https://ru.wowhead.com/spell=475) **Мага**.
  * [Каменная форма](https://ru.wowhead.com/spell=20594) **Дворфов** (только с себя).
  * [Огненная кровь](https://ru.wowhead.com/spell=265221) **Дворфов из клана Черного Железа** (только с себя).
* Диспелы следующих классов **не могут** снимать проклятия (даже в спеке хила):
  * [Очищение от болезни](https://ru.wowhead.com/spell=213634) **Жреца**.
  * [Детоксикация](https://ru.wowhead.com/spell=218164) **Монаха**.
  * [Очищение от токсинов](https://ru.wowhead.com/spell=213644/) **Паладина**.
* В Мифик+ обязательно необходимо снимать следующие проклятия:
  * **Та Сторона**:
    * [Сонная мерцающая пыльца](https://ru.wowhead.com/spell=334493) **Чащобного мерцающего мотылька**.
  * **Чертоги Покаяния**:
    * [Проклятие уничтожения](https://ru.wowhead.com/spell=325876) **Порочного уничтожителя**, если не сбили каст.
    * [Проклятие камня](https://ru.wowhead.com/spell=319603) **Эшелона**. В первую очередь стоит диспелить союзников, ставших целью способности [Разбивающий камень прыжок](https://ru.wowhead.com/spell=319592/).
  * **Туманы Тирна Скитта**:
    * [Последний вздох](https://ru.wowhead.com/spell=322968) после смерти **Друста-злобный коготь**, если вы до этого не сняли с них бафф с помощью [Развеивания магии](https://ru.wowhead.com/spell=370).
  * **Кровавые Катакомбы**:
    * [Подавляющее проклятье](https://ru.wowhead.com/spell=326836) **Зловещей подавительницы**, если не сбили каст.
    * [Взрывоопасный гнев](https://ru.wowhead.com/spell=336277/) **Частицы неистовства** стоит диспелить перед самым спадением, так как это проклятие увеличивает наносимый союзником урон, но взрывается в АоЕ при спадении без диспела.
    * [Запятнанная грехом анима](https://ru.wowhead.com/spell=328494) **Вершителя Тарвольда**, если союзник задел движущийся красный круг на полу.
  * **Театр Боли**:
    * [Уязвимость к темной магии](https://ru.wowhead.com/spell=330725/) **Стража портала**, чтобы не получать увеличенный урон от [Бури душ](https://ru.wowhead.com/spell=330716).
    * [Проклятие запустения](https://ru.wowhead.com/spell=333299) **Пагубного темного проповедника**.
    * [Хватка смерти](https://ru.wowhead.com/spell=323831) **Мордреты, Вечной императрицы**, если игрока затянуло в область [Затягивающего разлома](https://ru.wowhead.com/spell=323685/).

 
<a href="https://ru.wowhead.com/spell=8143" target="blank" data-wh-icon-size="small" >**Тотем трепета**</a> – снимает с союзников эффекты страха, подчинения и сна.

> Tremor Totem, Тремор, Антифир. 

* При использовании призывается тотем под ноги персонажа, который снимает с союзников эффекты страха, подчинения и сна.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **2.3%** маны.
* Снимает эффекты раз в **1** секунду, начиная с момента установки.
* Длительность составляет **10** секунд
* Радиус действия – **30** метров вокруг тотема.
* Время восстановления – **1** минута.
* Проводник [Сила тотемов](https://ru.wowhead.com/spell=338042/%D1%81%D0%B8%D0%BB%D0%B0-%D1%82%D0%BE%D1%82%D0%B5%D0%BC%D0%BE%D0%B2?ilvl=226) снижает время восстановления дополнительно на **7** секунд, до **53** секунд.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Единственный аналог есть у Паладина – проводник [Чистое сосредоточение](https://ru.wowhead.com/spell=339124/%D1%87%D0%B8%D1%81%D1%82%D0%BE%D0%B5-%D1%81%D0%BE%D1%81%D1%80%D0%B5%D0%B4%D0%BE%D1%82%D0%BE%D1%87%D0%B5%D0%BD%D0%B8%D0%B5?ilvl=226), который снижает время действия эффектов страха, подчинения и сна на **32%**.
* В Мифик+ желательно ставить [Тотем трепета](https://ru.wowhead.com/spell=8143) под следующие заклинания:
  * **Та Сторона**:
    * [Горестный вой](https://ru.wowhead.com/spell=340026/) **Мифреш Небесный Коготь**, если вы зачем-то запулили этого дракона.
  * **Чертоги Покаяния**:
    * [Видения греховного света](https://ru.wowhead.com/spell=339237) **Халкиаса**, если игрок покинул область действия [Света покаяния](https://ru.wowhead.com/spell=339235).
  * **Туманы Тирна Скитта**:
    * [Отвратительная личина](https://ru.wowhead.com/spell=328756) **Ингра Малох**, во время трехсекундного каста.
  * **Смертельная Тризна**:
    * [Дребезжащий крик](https://ru.wowhead.com/spell=324293) **Скетела-мародера**, если не сбили каст.
  * **Театр Боли**:
    * [Проклятие запустения](https://ru.wowhead.com/spell=333299) **Пагубного темного проповедника**, если не сняли проклятие с союзника с помощью [Очищения духа](https://ru.wowhead.com/spell=51886).

<a href="https://ru.wowhead.com/spell=192077" target="blank" data-wh-icon-size="small" >**Тотем ветряного порыва**</a> – талант пятого тира, увеличивает скорость передвижения союзников.

> Wind Rush Totem, WRT, ВРТ, Хаст тотем, Тотем ветроступа, Тотем на скорость.

* При использовании устанавливается тотем в выбранном месте, увеличивающий скорость передвижения союзников на **60%**.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **2** минуты. Оно не снижается от проводника [Сила тотемов](https://ru.wowhead.com/spell=338042/%D1%81%D0%B8%D0%BB%D0%B0-%D1%82%D0%BE%D1%82%D0%B5%D0%BC%D0%BE%D0%B2?ilvl=226).
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Длительность действия баффа на скорость передвижения – **5** секунд.
* Бафф обновляется каждую секунду, пока вы находитесь в области действия тотема.
* Радиус действия – **10** метров.
* Сам тотем существует **15** секунд.
* Можно установить в любую точку на расстоянии **40** метров от персонажа.
* Для быстрой установки [Тотема ветряного порыва](https://ru.wowhead.com/spell=192077) под персонажа можно использовать следующий макрос:
~~~
#showtooltip
/cast [@player] Тотем ветряного порыва
~~~
* Аналог подобного ускорения – [Тревожный рев](https://ru.wowhead.com/spell=106898) **Друида**. Он требует находиться в радиусе **15** метров от Друида во время каста и не дает бафф, если союзник был дальше этого расстояния, однако имеет большую длительность – **8** секунд. Проводник [Во главе стаи](https://ru.wowhead.com/spell=341450/%D0%B2%D0%BE-%D0%B3%D0%BB%D0%B0%D0%B2%D0%B5-%D1%81%D1%82%D0%B0%D0%B8?ilvl=226) увеличивает расстояние до **18.6** метров и длительность до **10** секунд. 


<a href="https://ru.wowhead.com/spell=51490" target="blank" data-wh-icon-size="small" >**Гром и молния**</a> – АоЕ отбрасывание.

> Thunderstorm, Knockback, Нокбек, Тандер, Откидывание, Отброс.  

* При использовании отбрасывает противников в стороны от персонажа.
* Наносит **10%** урона от силы заклинаний.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **45** секунд.
* При использовании откидывает противников на расстоянии **10** метров от персонажа на **20** метров.
* После откидывания замедляет пораженных противников на **40%** на **5** секунд.
* Можно применять во время оглушения.
* Крайне полезное заклинание в Мифик+ на [Кровавом](https://ru.wowhead.com/affix=8) и [Злопамятном](https://ru.wowhead.com/affix=123) аффиксах.
* Также может пригодиться, если на вас сагрились мобы, с помощью способности [Гром и молния](https://ru.wowhead.com/spell=51490) их можно откинуть обратно к танку.
* Можно использовать для прерывания заклинаний, в том числе для тех, которые нельзя сбить обычным способом.
* Не работает на боссов.
* Аналоги [Грома и молния](https://ru.wowhead.com/spell=51490) у других классов:
  * [Тайфун](https://ru.wowhead.com/spell=132469) **Друида** (базово у Баланса, у других спеков через талант [Родство с балансом](https://ru.wowhead.com/spell=197488)).
  * [Взрывной выстрел](https://ru.wowhead.com/spell=186387) **Охотника** (Стрельба).
  * [Хватка смерти](https://ru.wowhead.com/spell=49576) **Рыцаря смерти**.
  * [Хватка Кровожада](https://ru.wowhead.com/spell=108199/) **Рыцаря смерти** (Кровь).
  * [Рука поганища](https://ru.wowhead.com/spell=315443) **Рыцаря смерти** (Некролорд).
  * [Круг мира](https://ru.wowhead.com/spell=116844) **Монаха** (талант).
  * [Печать цепей](https://ru.wowhead.com/spell=202138/) **Охотника на демонов** (Месть, талант).
  * [Сияющая мощь](https://ru.wowhead.com/spell=204263) **Жреца** (Послушание и Свет, талант).
  * [Взрывная волна](https://ru.wowhead.com/spell=157981) **Мага** (Огонь, талант).


<a href="https://ru.wowhead.com/spell=57994" target="blank" data-wh-icon-size="small" >**Пронизывающий ветер**</a> – прерывание заклинания противника.

> Wind Shear, WS, Интерапт, Сбивалка, Ветер, Кик.

* Не имеет ГКД.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Время восстановления – **12** секунд, **самый короткий КД интерапта в игре**.
* Радиус действия [Пронизывающего ветра](https://ru.wowhead.com/spell=57994) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* После сбивания не дает противнику кастовать заклинание той же школы магии в течение **3** секунд.
* При использовании [Пронизывающего ветра](https://ru.wowhead.com/spell=57994) во время произнесения другого заклинания оно будет прервано (подобно команде /stopcasting).
* Аналоги [Пронизывающего ветра](https://ru.wowhead.com/spell=57994) у других классов:
  * [Зуботычина](https://ru.wowhead.com/spell=6552) **Воина** (все спеки, **15** секунд КД, ближний бой).
  * [Пинок](https://ru.wowhead.com/spell=1766) **Разбойника** (все спеки, **15** секунд КД, ближний бой).
  * [Встречный выстрел](https://ru.wowhead.com/spell=147362) **Охотника** (ММ и БМ, **24** секунды КД, **40** метров).
  * [Намордник](https://ru.wowhead.com/spell=187707) **Охотника** (Выживание, **15** секунд КД, ближний бой).
  * [Рука-копье](https://ru.wowhead.com/spell=116705) **Монаха** (Танцующий с ветром и Хмелевар, **15** секунд КД, ближний бой).
  * [Укор](https://ru.wowhead.com/spell=96231) **Паладина** (Воздаяние и Защита, **15** секунд КД, ближний бой).
  * [Прерывание](https://ru.wowhead.com/spell=183752) **Охотника на демонов** (все спеки, **15** секунд КД, **15** метров).
  * [Заморозка разума](https://ru.wowhead.com/spell=47528) **Рыцаря смерти** (все спеки, **15** секунд КД, **15** метров).
  * [Щит мстителя](https://ru.wowhead.com/spell=31935) **Паладина** (Защита, **15** секунд КД, **30** метров).
  * [Божественный благовест](https://ru.wowhead.com/spell=304971) **Паладина** (Кирии, Защита, **60** секунд КД, **30** метров).
  * [Лобовая атака](https://ru.wowhead.com/spell=106839) **Друида** (Сила зверя и Страж, **15** секунд КД, **13** метров).
  * [Столп солнечного света](https://ru.wowhead.com/spell=78675) **Друида** (Баланс, **60** секунд КД, **40** метров, сбивает в радиусе **5** метров).
  * [Антимагия](https://ru.wowhead.com/spell=2139) **Мага** (**24** секунд КД, **40** метров).
  * [Безмолвие](https://ru.wowhead.com/spell=15487) **Жреца** (Тьма, **45** секунд КД (**30** с талантом), **30** метров).
  * [Запрет чар](https://ru.wowhead.com/spell=19647) **Чернокнижника** (Питомец, **24** секунды КД, **40** метров).

<a href="https://ru.wowhead.com/spell=51514" target="blank" data-wh-icon-size="small" >**Сглаз**</a> – долгий контроль гуманоида или зверя.

> Hex, Хекс, Лягушка, Контроль, Полиморф шамана.  

* При использовании превращает цель в зверька, не позволяя совершать какие-либо действия, кроме медленного передвижения.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Превращает цель в лягушку (или другую форму) на **60** секунд, не позволяя совершать какие-либо действия.
* В PvP базовое время действия составляет **8** секунд, без учета «Diminishing returns».
* Является Проклятием, а не Магией.
* Время восстановления **20** секунд (только после **56** уровня и открытия пассивной способности [Сглаз: Ранг 2](https://ru.wowhead.com/spell=343198), до этого **30** сек).
* Время произнесения – **1.7** секунд.
* Радиус действия [Сглаза](https://ru.wowhead.com/spell=51514) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* Работает только на **гуманоидов** и **зверей**.
* Не сбивается уроном мгновенно, в отличие от других подобных заклинаний.
* При использовании [Сглаза](https://ru.wowhead.com/spell=51514) вне боя не вводит в комбат, как и другие эффекты долгого контроля.
* Не уменьшает агро-радиус цели, в отличие от [Пленения](https://ru.wowhead.com/spell=217832) Охотника на демонов и [Ошеломления](https://ru.wowhead.com/spell=6770) Разбойника.
* Имеет множество различных форм, способы открытия есть по ссылке на Wowhead:
  * [Лягушка](https://ru.wowhead.com/spell=51514)
  * [Компсогнат](https://ru.wowhead.com/spell=210873)
  * [Змея](https://ru.wowhead.com/spell=211010)     
  * [Детеныш-скелет](https://ru.wowhead.com/spell=269352)
  * [Плетеный кусака](https://ru.wowhead.com/spell=277784)
  * [Сгусток живого меда](https://ru.wowhead.com/spell=309328)
  * [Зандаларский жилорез](https://ru.wowhead.com/spell=277778)
  * [Таракан](https://ru.wowhead.com/spell=211015)
  * [Паук](https://ru.wowhead.com/spell=211004)          
* Аналоги [Сглаза](https://ru.wowhead.com/spell=51514) у других классов:
  * [Пленение](https://ru.wowhead.com/spell=217832) **Охотника на демонов** (Гуманоиды, Животные и Демоны).
  * [Ошеломление](https://ru.wowhead.com/spell=6770) **Разбойника** (Гуманоиды, Животные, Демоны и Драконы).
  * [Превращение](https://ru.wowhead.com/spell=118) **Мага** (Гуманоиды, Животные и Зверьки).
  * [Спячка](https://ru.wowhead.com/spell=2637) **Друида** (Животные и Драконы).
  * [Замораживающая ловушка](https://ru.wowhead.com/spell=187650) **Охотника** (все типы существ).
  * [Отпугивание](https://ru.wowhead.com/spell=1513) **Охотника** (только Животные).
  * [Паралич](https://ru.wowhead.com/spell=115078) **Монаха** (все типы существ).
  * [Покаяние](https://ru.wowhead.com/spell=20066) **Паладина** (Гуманоиды, Демоны, Нежить, Драконы и Великаны).
  * [Сковывание нежити](https://ru.wowhead.com/spell=9484) **Жреца** (только Нежить).
  * [Изгнание](https://ru.wowhead.com/spell=710) **Чернокнижника** (Демоны, Аберрации и Элементали).
  * [Страх](https://ru.wowhead.com/spell=5782) **Чернокнижника** (все типы существ).




<a href="https://ru.wowhead.com/spell=192058" target="blank" data-wh-icon-size="small" >**Тотем конденсации**</a> – АоЕ стан.

* При использовании устанавливается тотем в выбранном месте, оглушающий ближайших врагов.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **10%** маны.
* Стан срабатывает не сразу, а спустя **2** секунды после установки тотема.
* Длительность оглушения – **3** секунды.
* Радиус действия – **8** метров вокруг тотема.
* Радиус установки тотема – **40** метров вокруг персонажа.
* Время восстановления – **1** минута.
* Талант [Статический разряд](https://ru.wowhead.com/spell=265046) сокращает время восстановления [Тотема конденсации](https://ru.wowhead.com/spell=192058) на **5** секунд за каждого оглушенного им противника, максимум **4** цели или **20** секунд сокращения КД.
* Проводник [Сила тотемов](https://ru.wowhead.com/spell=338042/%D1%81%D0%B8%D0%BB%D0%B0-%D1%82%D0%BE%D1%82%D0%B5%D0%BC%D0%BE%D0%B2?ilvl=226) снижает время восстановления дополнительно на **7** секунд, до **53** секунд.
* Имеет укороченный ГКД в **1** секунду, и это время не снижается Скоростью.
* Аналоги [Тотем конденсации](https://ru.wowhead.com/spell=192058) у других классов:
  * [Круговой удар ногой](https://ru.wowhead.com/spell=119381) **Монаха** (**60** секунд КД, **3** секунды длительности).
  * [Неистовство Тьмы](https://ru.wowhead.com/spell=30283) **Чернокнижника** (**60** секунд КД, **3** секунды длительности).
  * [Ударная волна](https://ru.wowhead.com/spell=46968) **Воина** (Защита, **40** секунд КД, **2** секунды длительности).
  * [Кольцо хаоса](https://ru.wowhead.com/spell=344867) **Охотника на демонов** (**60** секунд КД, **2** секунды длительности).


<a href="https://ru.wowhead.com/spell=2645" target="blank" data-wh-icon-size="small" >**Призрачный волк**</a> – превращение в призрачного волка, который увеличивает скорость передвижения.

> Ghost Wolf, Волк, Форма волка, Пес, Раптор, Динозавр.  

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Увеличивает скорость передвижения на **30%**.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) скорость передвижения не может быть снижена менее чем до **100%**.
* Применение любого заклинания (кроме [Хождения по воде](https://ru.wowhead.com/spell=546) и [Астрального сдвига](https://ru.wowhead.com/spell=108271)) отменяет форму [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Не имеет времени восстановления.
* При входе в форму тратится ГКД, а при выходе из формы – нет.
* Форма [Призрачного волка](https://ru.wowhead.com/spell=2645) считается магическим эффектом и может быть рассеяна соответствующими заклинаниеми, типа [Развеивания магии](https://ru.wowhead.com/spell=370), [Рассеивания заклинаний](https://ru.wowhead.com/spell=528/) и других. [Чарокрад](https://ru.wowhead.com/spell=30449) Мага при этом снимет эффект формы, но не получит этот эффект для себя.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) вы считаетесь **Животным**, а не **Гуманоидом**:
  * Охотники видят вас на миникарте с помощью [Выслеживания животных](https://ru.wowhead.com/spell=210065) и могут законтролить с помощью [Отпугивания](https://ru.wowhead.com/spell=1513), а Друид – с помощью [Спячки](https://ru.wowhead.com/spell=2637).
  * Вас нельзя отследить с помощью [Выслеживания гуманоидов](https://ru.wowhead.com/spell=19883), а Паладин не сможет законтролить с помощью [Покаяния](https://ru.wowhead.com/spell=20066).
* Форму [Призрачного волка](https://ru.wowhead.com/spell=2645) можно принимать в облике духа после смерти, что позволит быстрее добраться до своего тела.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно собирать траву.
* Талант [Дух волка](https://ru.wowhead.com/spell=260878) дополнительно повышает скорость передвижения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645) на **20%**, а также дает снижение получаемого урона на **20%**. Эффект накапливается по **5%** за одну секунду нахождения в форме [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Внешний вид формы [Призрачного волка](https://ru.wowhead.com/spell=2645) можно изменить с помощью символов, которые делаются начертателями и покупаются на аукционе:
  * [Символ призрачного ящера](https://ru.wowhead.com/item=137287)
  * [Символ волка-призрака](https://ru.wowhead.com/item=43386/)
* В бою против Сира Денатрия иммунитет к снижению скорости передвижения позвляет не снимать стаки [Бремени греха](https://ru.wowhead.com/spell=326699), что помогает при переходе на вторую фазу – [Марш кающихся](https://ru.wowhead.com/spell=328276/). Главное – принять форму [Призрачного волка](https://ru.wowhead.com/spell=2645) до того, как у босса станет **70.5%** здоровья.
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно танцевать, с помощью команды «/dance».
* В форме [Призрачного волка](https://ru.wowhead.com/spell=2645) можно атаковать, нанося при этом базовый урон от оружия.

<a href="https://ru.wowhead.com/spell=192106" target="blank" data-wh-icon-size="small" >**Щит молний**</a> – наносит урон атакующим в ближнем бою.

>  Lightning Shield, LS, ЛС, Щит.  

* При использовании вы получаете бафф на **30** минут.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **1.5%** маны.
* Атакующие вас в ближнем бою противники получают **15%** урона от силы заклинаний.
* Каждый удар по вам в ближнем бою дает **5** ед. энергии Водоворота.
* Возможно поддерживать только один щит стихий одновременно – либо [Щит молний](https://ru.wowhead.com/spell=192106), либо [Щит земли](https://ru.wowhead.com/spell=974).
* Щит не спадает при смерти.
* Дает анимацию электрического шара, кружащего вокруг персонажа.

<a href="https://ru.wowhead.com/spell=2008" target="blank" data-wh-icon-size="small" >**Дух предков**</a> – воскрешение союзника вне боя.

> Ancestral Spirit, Рес.

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и стоит **4%** маны.
* Время произнесения – **10** секунд.
* Воскрешается **1** союзник с **35%** здоровья и маны.
* Не имеет времени восстановления.
* Радиус воскрешения **40** метров, тогда как у массового воскрешения Рестора, [Древнее видение](https://ru.wowhead.com/spell=212048), **100** метров.
* Символ, который добавляет анимацию духа предков после воскрешения, – [Символ далеких предков](https://ru.wowhead.com/item=104127).

<a href="https://ru.wowhead.com/spell=546" target="blank" data-wh-icon-size="small" >**Хождение по воде**</a>  – позволяет союзнику ходить по воде.

* При использовании накладывает бафф на **10** минут, который позволяет ходить по воде.
* Любой урон отменяет этот эффект.
* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Можно использовать не выходя из формы [Призрачного волка](https://ru.wowhead.com/spell=2645).
* Кастуется мгновенно и имеет ГКД.
* Радиус действия [Хождения по воде](https://ru.wowhead.com/spell=546) меньше, чем у других заклинаний – **30** метров вместо **40** метров.
* ГКД от этой способности не распространяется на призыв маунтов. Поэтому можно использовать одновременно {{ site.data.spells.waterwalking }} и маунта: 
~~~
#showtooltip 
/cast [nomounted,nocombat] Хождение по воде
/run C_MountJournal.SummonByID(0)
~~~

<a href="https://ru.wowhead.com/spell=6196" target="blank" data-wh-icon-size="small" >**Дальнее зрение**</a> – переносит точку обзора в указанное место.

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Кастуется **2** секунды и длится **1** минуту.
* После каста можно применить это заклинание еще раз, переместив точку обзора в следующее место.
* Практически не имеет ограничения на дальность применения.
* Может найти применение в Башне Торгаст, так как позволяет найти кратчайший путь до выхода стоя у входа на этаж.

<a href="https://ru.wowhead.com/spell=556" target="blank" data-wh-icon-size="small" >**Астральное возвращение**</a> – дополнительный [Камень возвращения](https://ru.wowhead.com/item=6948), возвращает вас в привязанную локацию.   

* Считается заклинанием магии <span style="color:#26d22b;font-size:1em;">**Природы**</span> и не стоит маны.
* Возвращает в ту же точку, куда назначен [Камень возвращения](https://ru.wowhead.com/item=6948), но позволяет делать это чаще.
* Имеет раздельное время восстановления с [Камнем возвращения](https://ru.wowhead.com/item=6948).
* Кастуется **10** секунд, но, в отличие от [Камня возвращения](https://ru.wowhead.com/item=6948), это время сокращается за счет Скорости.
* Можно использовать на ходу во время действия [Благосклонности предков](https://ru.wowhead.com/spell=79206).
* Время восстановления – **10** минут.
* Нельзя использовать в бою и на арене.

<a href="https://ru.wowhead.com/spell=73899" target="blank" data-wh-icon-size="small" >**Стихийный удар**</a> – бесполезная способность для шамана Стихии. 

* Данная способность нужна только для того, чтобы показать весь арсенал шамана в самом начале прокачки, на уровнях с **1** по **10**.
* После получения **10** уровня, выбрав специализацию, шаман Стихии переходит на дальний бой и про [Стихийный удар](https://ru.wowhead.com/spell=73899) можно забыть.

<a href="https://ru.wowhead.com/spell=318038" target="blank" data-wh-icon-size="small" >**Оружие языка пламени**</a> – бесполезная способность для Шамана Стихии.

* Не дает использовать масло на оружие, например [Масло теневого ядра](https://ru.wowhead.com/item=171285).
* Добавляет огненную анимацию вокруг оружия персонажа.
* Данная способность нужна только для того, чтобы показать весь арсенал шамана в самом начале прокачки, на уровнях с **1** по **10**.
* После получения **10** уровня, выбрав специализацию, шаман Стихии переходит на дальний бой и про [Оружие языка пламени](https://ru.wowhead.com/spell=318038) можно забыть.

<a href="https://ru.wowhead.com/spell=137040" target="blank" data-wh-icon-size="small" >**Шаман - Стихии**</a> – скрытая аура, используется как модификатор урона и затраты ресурсов для различных заклинаний.

* С помощью таких аур разработчики могут менять показатели урона сразу всех способностей специализации, не меняя значения «урона от силы заклинаний» для каждой по отдельности.
* Например, по ауре шамана Стихии видно, что у Элем шамана [Исцеляющий всплеск](https://ru.wowhead.com/spell=8004) хилит на **13%** больше, но стоит на **9%** больше маны.
* Аналогично с [Щитом земли](https://ru.wowhead.com/spell=974), который у Элема хилит на **68%** сильнее, чем у Рестора.


