...
* {{ site.data.spells.ls }} – больше не талант в первом тире, а бафф длительностью 30 мин. Данный щит будет полезен на прокачке и в ПВП, но не за счет наносимого урона (который достаточно слаб), а за счёт генерации зарядов {{ site.data.spells.mw }}. Не сочетается с Земляным щитом.
...
* {{ site.data.spells.lb }} – {{ site.data.spells.lb }} больше не мгновенная, а значит на прокачке подпуливать мобов мы будем шоками или {{ site.data.spells.purge }}. Кастуется исключительно с 5+ стаками {{ site.data.spells.mw }}, когда {{ site.data.spells.lb }} инстантная, потому что заклинания с кастом прерывают автоатаку, что ведет к потере ДПС.
...
* {{ site.data.spells.hst }} – стоит достаточно мало маны, а также мгновенно кастуется и имеет сниженный ГКД, как и у всех тотемов (1 сек.). Будет полезен на перебежках. Этим тотемом можно заполнять возникающие простои в ротации.
...
* {{ site.data.spells.hh }} (перемещен во второй тир) – теперь дает шанс получить бафф на 8 сек., который снижает кулдаун {{ site.data.spells.ll }} в 4 раза и ощутимо увеличивает его урон, делая его больше чем у {{ site.data.spells.ss }}.
...
Введение ограничения на количества целей на АОЕ затронуло Энха не так сильно, как многие другие спеки. Вот полный список количества поражаемых целей:
...
1. Используйте {{ site.data.spells.frs }} если у вас есть бафф от {{ site.data.spells.hailstorm }}.
2. Используйте {{ site.data.spells.windstrike }} если вы находитесь в {{ site.data.spells.asc_enh }}.
3. Используйте {{ site.data.spells.lb }} / {{ site.data.spells.cl }} если у вас 8 или более стаков {{ site.data.spells.mw }}.
4. Используйте {{ site.data.spells.ss }} по кулдауну, особенно при проке {{ site.data.spells.sb }}.
5. Используйте Дух дикого зверя для генерации стаков {{ site.data.spells.mw }}.
6. Поддерживайте {{ site.data.spells.fs }} на одной цели и держите шоки на кулдауне.
7. Используйте {{ site.data.spells.ll }}, особенно с баффом {{ site.data.spells.lf }}.
8. Используйте {{ site.data.spells.crush }}, даже в бою против одной цели.
