...
* **@cursor** – использование заклинание по области, на место в котором в данный момент находится курсор (доступно только для заклинаний с применением на область, например {{ site.data.spells.quake }} или {{ site.data.spells.rain }}).
...
Условные выражения используются для определения возможности действия. Вы можете добавить «no» перед ними, чтобы они вели себя прямо противоположным образом. Например, noharm верно для любой цели, которая не является врагом, но может включать например  нейтральных NPC, а nocombat верно если вы не находитесь в бою.
