---
title: Обзор Ковенантов – Рестор Шаман
layout: page
...
* Необходимо тратить ГКД для реализации саппорт потенциала этой способности. То есть Рестору нужно 3 раза использовать дамажащую способность за время действия тотема.  
...
* {{ site.data.conduits.dirge }} – проводник, усиливающий эту способность. Увеличивает наносимый урон и исцеление по одной цели, которая находится ближе всех к {{ site.data.covenants.vt }}. Полезно для отхила танка, а также для дпс в бою против одной цели.
* Аура на урон заклинаний – {{ site.data.spells.resto_passive }} не действует на {{ site.data.covenants.vt }}, хотя работает на другие способности ковенантов.  
...
{{ site.data.covenants.pw }} — излечивает одну цель, накладывая на неё {{ site.data.spells.riptide }}. Применяется мгновенно. Время восстановления 45 сек. Ваша следующая {{ site.data.spells.hw }} также применится ко всем целям под {{ site.data.spells.riptide }} с 60% эффективности.
...
* Полностью отсутствует саппорт потенциал. При применении к противник накладывает {{ site.data.spells.fs }}, но {{ site.data.spells.lvb }} не дублируется.
